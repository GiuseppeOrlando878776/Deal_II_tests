...
namespace Step35 {
  ...
  assemble_rhs_cell_term_projection_vorticity(const MatrixFree<dim, Number>&               data,
                                              ...
                                              const Vec&                                   src,
                                              const std::pair<unsigned int, unsigned int>& cell_range) const {
    FEEvaluation<dim, fe_degree_v, n_q_points_1d, dim, Number> phi(data, 0), phi_vel(data,0);
    ...
    for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
      ...
      phi.reinit(cell);
      for(unsigned int q = 0; q < phi.n_q_points; ++q) {
        Tensor<1, dim, VectorizedArray<Number>> rot_u;
        const auto& grad_u = phi_vel.get_gradient(q);
        rot_u[0] = grad_u[1][0] - grad_u[0][1];
  ...
  void NavierStokesProjection<dim>::project_vorticity() {
    ...
    const std::vector<unsigned int> tmp = {0};
    navier_stokes_matrix.initialize(matrix_free_storage, tmp, tmp);
    navier_stokes_matrix.vmult_vorticity_projection(rhs_u, u_n);
  ...
  void NavierStokesProjection<dim>::output_results(const unsigned int step) {
    project_vorticity();
    const FESystem<dim> joint_fe(fe_velocity, 1, fe_pressure, 1, fe_velocity, 1);
    DoFHandler<dim>     joint_dof_handler(triangulation);
    joint_dof_handler.distribute_dofs(joint_fe);
    Assert(joint_dof_handler.n_dofs() == (2*dof_handler_velocity.n_dofs() + dof_handler_pressure.n_dofs()),
           ExcInternalError());
      ...
      for(unsigned int i = 0; i < joint_fe.n_dofs_per_cell(); ++i) {
        switch(joint_fe.system_to_base_index(i).first.first) {
          ...
          case 2:
            Assert(joint_fe.system_to_base_index(i).first.second < dim,
                   ExcInternalError());
            joint_solution(loc_joint_dof_indices[i]) = u_tmp(loc_vel_dof_indices[joint_fe.system_to_base_index(i).second]);
            break;
          default:
    ...
    component_interpretation(2*dim + 1, DataComponentInterpretation::component_is_part_of_vector);
