...
namespace Step35 {
  ...
  protected:
    ...
    LinearAlgebra::distributed::Vector<double> rhs_rho_tmp;
    LinearAlgebra::distributed::Vector<double> rhs_rho_tmp_1;
  ...
  void NavierStokesProjection<dim>::setup_dofs() {
    ...
    matrix_free_storage->initialize_dof_vector(rhs_rho_tmp, 2);
    matrix_free_storage->initialize_dof_vector(rhs_rho_tmp_1, 2);
  ...
  void NavierStokesProjection<dim>::update_density() {
    ...
    if(HYPERBOLIC_stage == 1) {
      ...
      FETools::interpolate(dof_handler_density_Q0, rhs_rho_Q0, dof_handler_density, rhs_rho_tmp);

      /*auto it1 = dof_handler_density.begin_active();
      auto it2 = dof_handler_density_Q0.begin_active();
      while(it1 != dof_handler_density.end()) {
        if(it1->is_locally_owned()) {
          std::vector<types::global_dof_index> dof_indices_Q0(fe_density_Q0.dofs_per_cell);
          it2->get_dof_indices(dof_indices_Q0);

          std::vector<types::global_dof_index> dof_indices(fe_density.dofs_per_cell);
          it1->get_dof_indices(dof_indices);
          for(unsigned int idx = 0; idx < dof_indices.size(); ++idx)
            rhs_rho_tmp(dof_indices[idx]) = rhs_rho_Q0(dof_indices_Q0[0]);
        }
        ++it1;
        ++it2;
      }*/
      ...
      FETools::interpolate(dof_handler_density_Q0, rhs_rho_Q0_contrib1, dof_handler_density, rhs_rho_tmp);
      ...
      FETools::interpolate(dof_handler_density_Q0, rhs_rho_Q0_contrib2, dof_handler_density, rhs_rho_tmp_1);
    }
    if(HYPERBOLIC_stage == 1) {
      rhs_rho.add(-1.0, rhs_rho_tmp);
      rhs_rho /= (10.0*GridTools::minimal_cell_diameter(triangulation)/std::sqrt(dim)*gamma*dt);
      for(unsigned int i = 0; i < rhs_rho.local_size(); ++i)
        rhs_rho.local_element(i) *= (std::abs(rhs_rho.local_element(i)) <= 1.0);
      rhs_rho *= (10.0*GridTools::minimal_cell_diameter(triangulation)/std::sqrt(dim)*gamma*dt);
      rhs_rho.add(1.0, rhs_rho_tmp);
    }
    else {
      rhs_rho_contrib1.add(-1.0, rhs_rho_tmp);
      rhs_rho_contrib1 /= (10.0*GridTools::minimal_cell_diameter(triangulation)/std::sqrt(dim)*dt);
      for(unsigned int i = 0; i < rhs_rho_contrib1.local_size(); ++i)
        rhs_rho_contrib1.local_element(i) *= (std::abs(rhs_rho_contrib1.local_element(i)) <= 1.0);
      rhs_rho_contrib1 *= (10.0*GridTools::minimal_cell_diameter(triangulation)/std::sqrt(dim)*dt);
      rhs_rho_contrib1.add(1.0, rhs_rho_tmp);

      rhs_rho.equ(a31, rhs_rho_contrib1);

      rhs_rho_contrib2.add(-1.0, rhs_rho_tmp_1);
      rhs_rho_contrib2 /= (10.0*GridTools::minimal_cell_diameter(triangulation)/std::sqrt(dim)*dt);
      for(unsigned int i = 0; i < rhs_rho_contrib2.local_size(); ++i)
        rhs_rho_contrib2.local_element(i) *= (std::abs(rhs_rho_contrib2.local_element(i)) <= 1.0);
      rhs_rho_contrib2 *= (10.0*GridTools::minimal_cell_diameter(triangulation)/std::sqrt(dim)*dt);
      rhs_rho_contrib2.add(1.0, rhs_rho_tmp_1);

      rhs_rho.add(a32, rhs_rho_contrib2);
    }
  ...
  void NavierStokesProjection<dim>::run(const bool verbose, const unsigned int output_interval) {
    ...
    while(std::abs(T - time) > 1e-10) {
      ...
      FETools::interpolate(dof_handler_density, rho_tmp_2, dof_handler_density_Q0, rho_tmp_2_Q0);
      ...
      FETools::interpolate(dof_handler_density, rho_old, dof_handler_density_Q0, rho_old_Q0);
