...
namespace Step35 {
    ...
    if(HYPERBOLIC_stage == 1) {
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          phi.submit_gradient(a32*a21/a31*dt*phi_rho_tmp_2.get_value(q)*phi_u_tmp_2.get_value(q), q);
        }
        phi.integrate_scatter(true, true, dst);
      }
    }
    ...
    if(HYPERBOLIC_stage == 1) {
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi_p.n_q_points; ++q) {
          ...
          const auto  lambda   = std::max(std::abs(scalar_product(phi_u_old_p.get_value(q), n_plus)),
      ...
      FEFaceEvaluation<dim, fe_degree_u, n_q_points_1d_u, dim, Number> phi_u_tmp_2_p(data, true, 0),
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        phi_u_tmp_2_p.gather_evaluate(src[1], true, false);
        phi_u_tmp_2_m.reinit(face);
        phi_u_tmp_2_m.gather_evaluate(src[1], true, false);
        phi_p.reinit(face);
        phi_m.reinit(face);
  ...
  assemble_rhs_boundary_term_rho_projection(const MatrixFree<dim, Number>&               data,
    ...
    if(HYPERBOLIC_stage == 1) {
      ...
      FEFaceEvaluation<dim, fe_degree_u, n_q_points_1d_u, dim, Number> phi_u_tmp_2(data, true, 0);
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& u_tmp_2_D      = phi_u_tmp_2.get_value(q);

          const auto& avg_flux_tmp_2 = 0.5*(phi_rho_tmp_2.get_value(q)*phi_u_tmp_2.get_value(q) + rho_tmp_2_D*u_tmp_2_D);

          phi.submit_value(-a32*a21/a31*dt*scalar_product(avg_flux_tmp_2, n_plus), q);
        }
        phi.integrate_scatter(true, false, dst);
      }
    }
    ...
    if(HYPERBOLIC_stage == 1) {
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& pres_tmp_2_D       = rho_tmp_2_D*
                                           (E_tmp_2_D - 0.5*Ma*Ma*scalar_product(u_tmp_2_D, u_tmp_2_D))*(EquationData::Cp_Cv - 1.0);
          const auto& avg_enthalpy_tmp_2 = 0.5*
  ...
  assemble_boundary_term_enthalpy(const MatrixFree<dim, Number>&               data,
    ...
    for(unsigned int face = face_range.first; face < face_range.second; ++face) {
      ...
      for(unsigned int q = 0; q < phi.n_q_points; ++q) {
        ...
        const auto& pres_fixed        = phi_pres_fixed.get_value(q);
        const auto& u_D               = phi_src.get_value(q);

        const auto& avg_flux_enthalpy = 0.5*EquationData::Cp_Cv/(EquationData::Cp_Cv - 1.0)*pres_fixed*(phi_src.get_value(q) + u_D);
    ...
    if(HYPERBOLIC_stage == 1) {
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& E_old                  = 1.0/(EquationData::Cp_Cv - 1.0)*(pres_old/rho_old)
                                             + 0.5*Ma*Ma*scalar_product(u_old, u_old);
          const auto& E_old_D                = E_old;
          const auto& pres_old_D             = rho_old_D*
                                               (E_old_D - 0.5*Ma*Ma*scalar_product(u_old_D, u_old_D))*(EquationData::Cp_Cv - 1.0);
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& pres_tmp_2_D               = rho_tmp_2_D*
                                                   (E_tmp_2_D -
                                                    0.5*Ma*Ma*scalar_product(u_tmp_2_D, u_tmp_2_D))*(EquationData::Cp_Cv - 1.0);
  ...
  assemble_rhs_face_term_rho_projection_Q0(const MatrixFree<dim, Number>&               data,
    ...
    if(HYPERBOLIC_stage == 1) {
      ...
      FEFaceEvaluation<dim, 0, n_q_points_1d_rho, 1, Number> phi_p(data, true, 3),
                                                             ...
                                                             phi_rho_old_p(data, true, 3),
                                                             phi_rho_old_m(data, false, 3),
                                                             phi_rho_tmp_2_p(data, true, 3),
                                                             phi_rho_tmp_2_m(data, false, 3);
      FEFaceEvaluation<dim, 0, n_q_points_1d_u, dim, Number> phi_u_old_p(data, true, 4),
                                                             phi_u_old_m(data, false, 4),
                                                             phi_u_tmp_2_p(data, true, 4),
                                                             phi_u_tmp_2_m(data, false, 4);
      FEFaceEvaluation<dim, 0, n_q_points_1d_rho, 1, Number> phi_pres_old_p(data, true, 3),
                                                             phi_pres_old_m(data, false, 3),
                                                             phi_pres_tmp_2_p(data, true, 3),
                                                             phi_pres_tmp_2_m(data, false, 3);
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        phi_u_tmp_2_m.gather_evaluate(src[1], true, false);
        phi_pres_tmp_2_p.reinit(face);
        phi_pres_tmp_2_p.gather_evaluate(src[2], true, false);
        phi_pres_tmp_2_m.reinit(face);
        phi_pres_tmp_2_m.gather_evaluate(src[2], true, false);
        phi_p.reinit(face);
        phi_m.reinit(face);
        ...
        for(unsigned int q = 0; q < phi_p.n_q_points; ++q) {
          ...
          const auto  lambda_tmp_2   = std::max(std::sqrt(scalar_product(phi_u_tmp_2_p.get_value(q), phi_u_tmp_2_p.get_value(q))) +
                                                1.0/Ma*
                                                std::sqrt(EquationData::Cp_Cv*phi_pres_tmp_2_p.get_value(q)/
                                                                              phi_rho_tmp_2_p.get_value(q)),
                                                std::sqrt(scalar_product(phi_u_tmp_2_m.get_value(q), phi_u_tmp_2_m.get_value(q))) +
                                                1.0/Ma*
                                                std::sqrt(EquationData::Cp_Cv*phi_pres_tmp_2_m.get_value(q)/
                                                                              phi_rho_tmp_2_m.get_value(q)));
          const auto& jump_rho_tmp_2 = phi_rho_tmp_2_p.get_value(q) - phi_rho_tmp_2_m.get_value(q);
  ...
  assemble_rhs_boundary_term_rho_projection_Q0(const MatrixFree<dim, Number>&               data,
    ...
    if(HYPERBOLIC_stage == 1) {
      ...
      FEFaceEvaluation<dim, 0, n_q_points_1d_rho, 1, Number> phi(data, true, 3),
                                                             phi_rho_tmp_2(data, true, 3);
      FEFaceEvaluation<dim, 0, n_q_points_1d_u, dim, Number> phi_u_tmp_2(data, true, 4);
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          phi.submit_value(-a32*a21/a32*dt*scalar_product(avg_flux_tmp_2, n_plus), q);
        }
        phi.integrate_scatter(true, false, dst);
      }
    }
  ...
  void NavierStokesProjection<dim>::update_density() {
    ...
    if(HYPERBOLIC_stage == 1) {
      ...
      euler_matrix.initialize(matrix_free_storage, tmp, tmp);
      euler_matrix.vmult_rhs_rho_projection(rhs_rho, {rho_tmp_2, u_tmp_2});

      euler_matrix.initialize(matrix_free_storage, tmp_Q0, tmp_Q0);
      euler_matrix.vmult_rhs_rho_projection_Q0(rhs_rho_Q0, {rho_tmp_2_Q0, u_tmp_2_Q0, pres_tmp_2_Q0});
    }
    ...
    if(HYPERBOLIC_stage == 1) {
      ...
      rho_tmp_2 /= (0.01*GridTools::minimal_cell_diameter(triangulation)/std::sqrt(dim)*gamma*dt);
      ...
      rho_curr /= (0.01*GridTools::minimal_cell_diameter(triangulation)/std::sqrt(dim)*a32*gamma/a31*dt);
