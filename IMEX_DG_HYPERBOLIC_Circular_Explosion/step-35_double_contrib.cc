...
namespace Step35 {
  ...
  namespace EquationData {
    ...
    double Pressure<dim>::value(const Point<dim>&  p, const unsigned int component) const {
      ...
      const double r  = std::sqrt((p[0] - 0.0)*(p[0] - 0.0) + (p[1] - 0.0)*(p[1] - 0.0));

      return 1.0*(r <= 0.5) + 0.1*(r > 0.5);
    }
  ...
  public:
    ...
    void set_Density_Contribution_stage(const unsigned int stage);
    ...
    void vmult_rhs_rho_projection_Q0(Vec& dst, const std::vector<Vec>& src) const;
  ...
  protected:
    ...
    unsigned int Density_contribution_stage;
  ...
  HYPERBOLICOperator(): MatrixFreeOperators::Base<dim, Vec>(), Ma(), Fr(), dt(), gamma(2.0 - std::sqrt(2.0)),
                        ...
                        a33_tilde(1.0 - std::sqrt(2)/2.0), HYPERBOLIC_stage(1), NS_stage(1), Density_contribution_stage(1) {}
  ...
  // Setter of Density stage (this can be known only during the effective execution
  // and so it has to be demanded to the class that really solves the problem)
  //
  template<int dim, int fe_degree_rho, int fe_degree_T, int fe_degree_u,
           int n_q_points_1d_rho, int n_q_points_1d_T, int n_q_points_1d_u, typename Vec, typename Number>
  void HYPERBOLICOperator<dim, fe_degree_rho, fe_degree_T, fe_degree_u,
                          n_q_points_1d_rho, n_q_points_1d_T, n_q_points_1d_u, Vec, Number>::
  set_Density_Contribution_stage(const unsigned int stage) {
    AssertIndexRange(stage, 3);
    Assert(stage > 0, ExcInternalError());
    Assert(HYPERBOLIC_stage == 2, ExcInternalError());
    Density_contribution_stage = stage;
  }
  ...
  assemble_rhs_cell_term_rho_projection(const MatrixFree<dim, Number>&               data,
    ...
    if(HYPERBOLIC_stage == 1) {
      ...
      FEEvaluation<dim, fe_degree_rho, n_q_points_1d_rho, 1, Number> phi(data, 2),
                                                                     phi_rho_old(data, 2);
      if(Density_contribution_stage == 1) {
        FEEvaluation<dim, fe_degree_u, n_q_points_1d_u, dim, Number> phi_u_old(data, 0);
        ...
        for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
          ...
          for(unsigned int q = 0; q < phi.n_q_points; ++q) {
            phi.submit_value(phi_rho_old.get_value(q), q);
            phi.submit_gradient(dt*phi_rho_old.get_value(q)*phi_u_old.get_value(q), q);
          }
          phi.integrate_scatter(true, true, dst);
        }
      }
      else {
        FEEvaluation<dim, fe_degree_rho, n_q_points_1d_rho, 1, Number> phi_rho_tmp_2(data, 2);
        FEEvaluation<dim, fe_degree_u, n_q_points_1d_u, dim, Number>   phi_u_tmp_2(data, 0);

        for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
          phi_rho_tmp_2.reinit(cell);
          phi_rho_tmp_2.gather_evaluate(src[0], true, false);
          phi_u_tmp_2.reinit(cell);
          phi_u_tmp_2.gather_evaluate(src[1], true, false);
          phi_rho_old.reinit(cell);
          phi_rho_old.gather_evaluate(src[2], true, false);
          phi.reinit(cell);

          for(unsigned int q = 0; q < phi.n_q_points; ++q) {
            phi.submit_value(phi_rho_old.get_value(q), q);
            phi.submit_gradient(dt*phi_rho_tmp_2.get_value(q)*phi_u_tmp_2.get_value(q), q);
          }
          phi.integrate_scatter(true, true, dst);
        }
      }
  ...
  assemble_rhs_face_term_rho_projection(const MatrixFree<dim, Number>&               data,
    ...
    if(HYPERBOLIC_stage == 1) {
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        phi_rho_p.gather_evaluate(src[0], true, false);
        phi_rho_m.reinit(face);
        phi_rho_m.gather_evaluate(src[0], true, false);
        phi_u_p.reinit(face);
        phi_u_p.gather_evaluate(src[1], true, false);
        phi_u_m.reinit(face);
        phi_u_m.gather_evaluate(src[1], true, false);
        phi_p.reinit(face);
        phi_m.reinit(face);
        ...
        for(unsigned int q = 0; q < phi_p.n_q_points; ++q) {
          ...
          const auto& avg_flux       = 0.5*(phi_rho_p.get_value(q)*phi_u_p.get_value(q) +
                                            phi_rho_m.get_value(q)*phi_u_m.get_value(q));
          const auto  lambda         = std::max(std::sqrt(scalar_product(phi_u_p.get_value(q), phi_u_p.get_value(q))),
                                                std::sqrt(scalar_product(phi_u_m.get_value(q), phi_u_m.get_value(q))));
          const auto& jump_rho       = phi_rho_p.get_value(q) - phi_rho_m.get_value(q);
  ...
  assemble_rhs_boundary_term_rho_projection(const MatrixFree<dim, Number>&               data,
    ...
    if(HYPERBOLIC_stage == 1) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          for(unsigned int v = 0; v < VectorizedArray<Number>::size(); ++v) {
            if(boundary_id == 0 || boundary_id == 1) {
              u_old_D[0][v] = phi_u_old.get_value(q)[0][v];
              u_old_D[1][v] = phi_u_old.get_value(q)[1][v];
            }
            else {
              u_old_D[0][v] = phi_u_old.get_value(q)[0][v];
              u_old_D[1][v] = phi_u_old.get_value(q)[1][v];
      ...
      FEFaceEvaluation<dim, fe_degree_rho, n_q_points_1d_rho, 1, Number> phi(data, true, 2),
                                                                         phi_rho(data, true, 2);
      FEFaceEvaluation<dim, fe_degree_u, n_q_points_1d_u, dim, Number>   phi_u(data, true, 0);
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& rho_D            = phi_rho.get_value(q);
          const auto& u_D              = phi_u.get_value(q);

          const auto& avg_flux         = 0.5*(phi_rho.get_value(q)*phi_u.get_value(q) + rho_D*u_D);

          phi.submit_value(-dt*scalar_product(avg_flux, n_plus), q);
        }
        phi.integrate_scatter(true, false, dst);
      }
    }
    ...
    if(HYPERBOLIC_stage == 1) {
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          auto        rho_old_D        = phi_rho_old.get_value(q);
          auto        pres_old_D       = phi_pres_old.get_value(q);
          auto        u_old_D          = Tensor<1, dim, VectorizedArray<Number>>();
          for(unsigned int v = 0; v < VectorizedArray<Number>::size(); ++v) {
            if(boundary_id == 0 || boundary_id == 1) {
              u_old_D[0][v] = phi_u_old.get_value(q)[0][v];
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          auto        pres_tmp_2_D     = phi_pres_tmp_2.get_value(q);
          auto        u_tmp_2_D        = Tensor<1, dim, VectorizedArray<Number>>();
          for(unsigned int v = 0; v < VectorizedArray<Number>::size(); ++v) {
            if(boundary_id == 0 || boundary_id == 1) {
              u_old_D[0][v]   = phi_u_old.get_value(q)[0][v];
              u_old_D[1][v]   = phi_u_old.get_value(q)[1][v];
              u_tmp_2_D[0][v] = phi_u_tmp_2.get_value(q)[0][v];;
              u_tmp_2_D[1][v] = phi_u_tmp_2.get_value(q)[1][v];
            }
            else {
              u_old_D[0][v]   = phi_u_old.get_value(q)[0][v];
              u_old_D[1][v]   = phi_u_old.get_value(q)[1][v];;
  ...
  assemble_boundary_term_enthalpy(const MatrixFree<dim, Number>&               data,
    ...
    for(unsigned int face = face_range.first; face < face_range.second; ++face) {
      ...
      for(unsigned int q = 0; q < phi.n_q_points; ++q) {
        ...
        const auto& pres_fixed        = phi_pres_fixed.get_value(q);
        auto        u_D               = Tensor<1, dim, VectorizedArray<Number>>();
        for(unsigned int v = 0; v < VectorizedArray<Number>::size(); ++v) {
          if(boundary_id == 0 || boundary_id == 1) {
            u_D[0][v] = phi_src.get_value(q)[0][v];
            u_D[1][v] = phi_src.get_value(q)[1][v];
          }
          else {
            u_D[0][v] = phi_src.get_value(q)[0][v];
            u_D[1][v] = phi_src.get_value(q)[1][v];
        ...
        const auto& avg_flux_enthalpy = 0.5*EquationData::Cp_Cv/(EquationData::Cp_Cv - 1.0)*pres_fixed*
                                        (phi_src.get_value(q) + u_D);
  ...
  assemble_boundary_term_pressure(const MatrixFree<dim, Number>&               data,
    ...
    for(unsigned int face = face_range.first; face < face_range.second; ++face) {
      ...
      for(unsigned int q = 0; q < phi.n_q_points; ++q) {
        ...
        phi.submit_value(coeff*dt/(Ma*Ma)*phi_src.get_value(q)*n_plus, q);
      }
      phi.integrate_scatter(true, false, dst);
    }
  }
  ...
  assemble_cell_term_rho_projection_Q0(const MatrixFree<dim, Number>&               data,
                                       ...
                                       const Vec&                                   src,
                                       const std::pair<unsigned int, unsigned int>& cell_range) const {
    FEEvaluation<dim, 0, n_q_points_1d_rho, 1, Number> phi(data, 3);
  ...
  assemble_rhs_cell_term_rho_projection_Q0(const MatrixFree<dim, Number>&               data,
    ...
    if(HYPERBOLIC_stage == 1) {
      FEEvaluation<dim, 0, n_q_points_1d_rho, 1, Number> phi(data, 3),
                                                         phi_rho_old(data, 3);
      FEEvaluation<dim, 0, n_q_points_1d_u, dim, Number> phi_u_old(data, 4);
      ...
      if(Density_contribution_stage == 1) {
        ...
        FEEvaluation<dim, 0, n_q_points_1d_u, dim, Number> phi_u_tmp_2(data, 4);
        FEEvaluation<dim, 0, n_q_points_1d_rho, 1, Number> phi_rho_tmp_2(data, 3);
        ...
        for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
          ...
          phi_u_tmp_2.gather_evaluate(src[1], true, false);
          phi_rho_old.reinit(cell);
          phi_rho_old.gather_evaluate(src[3], true, false);
          phi.reinit(cell);
  ...
  assemble_rhs_face_term_rho_projection_Q0(const MatrixFree<dim, Number>&               data,
                                           ...
                                           const std::pair<unsigned int, unsigned int>& face_range) const {
    if(HYPERBOLIC_stage == 1) {
      FEFaceEvaluation<dim, 0, n_q_points_1d_rho, 1, Number> phi_p(data, true, 3),
                                                             phi_m(data, false, 3),
                                                             phi_rho_old_p(data, true, 3),
                                                             phi_rho_old_m(data, false, 3);
      FEFaceEvaluation<dim, 0, n_q_points_1d_u, dim, Number> phi_u_old_p(data, true, 4),
                                                             phi_u_old_m(data, false, 4);
      FEFaceEvaluation<dim, 0, n_q_points_1d_rho, 1, Number> phi_pres_old_p(data, true, 3),
                                                             phi_pres_old_m(data, false, 3);
      ...
      FEFaceEvaluation<dim, 0, n_q_points_1d_rho, 1, Number> phi_p(data, true, 3),
                                                             phi_m(data, false, 3),
                                                             phi_rho_p(data, true, 3),
                                                             phi_rho_m(data, false, 3);
      FEFaceEvaluation<dim, 0, n_q_points_1d_u, dim, Number> phi_u_p(data, true, 4),
                                                             phi_u_m(data, false, 4);
      FEFaceEvaluation<dim, 0, n_q_points_1d_rho, 1, Number> phi_pres_p(data, true, 3),
                                                             phi_pres_m(data, false, 3);
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        phi_pres_p.gather_evaluate(src[2], true, false);
        phi_pres_m.reinit(face);
        phi_pres_m.gather_evaluate(src[2], true, false);
        phi_p.reinit(face);
        phi_m.reinit(face);
        ...
        for(unsigned int q = 0; q < phi_p.n_q_points; ++q) {
          ...
          const auto  lambda         = std::max(std::sqrt(scalar_product(phi_u_p.get_value(q), phi_u_p.get_value(q))) +
                                                1.0/Ma*
                                                std::sqrt(EquationData::Cp_Cv*phi_pres_p.get_value(q)/phi_rho_p.get_value(q)),
                                                std::sqrt(scalar_product(phi_u_m.get_value(q), phi_u_m.get_value(q))) +
                                                1.0/Ma*
                                                std::sqrt(EquationData::Cp_Cv*phi_pres_m.get_value(q)/phi_rho_m.get_value(q)));
          const auto& jump_rho       = phi_rho_p.get_value(q) - phi_rho_m.get_value(q);
  ...
  assemble_rhs_boundary_term_rho_projection_Q0(const MatrixFree<dim, Number>&               data,
                                               ...
                                               const std::pair<unsigned int, unsigned int>& face_range) const {
    if(HYPERBOLIC_stage == 1) {
      FEFaceEvaluation<dim, 0, n_q_points_1d_rho, 1, Number> phi(data, true, 3),
                                                             phi_rho_old(data, true, 3);
      FEFaceEvaluation<dim, 0, n_q_points_1d_u, dim, Number> phi_u_old(data, true, 4);
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& u_old_D          = phi_u_old.get_value(q);
      ...
      FEFaceEvaluation<dim, 0, n_q_points_1d_rho, 1, Number> phi(data, true, 3),
                                                             phi_rho(data, true, 3);
      FEFaceEvaluation<dim, 0, n_q_points_1d_u, dim, Number> phi_u(data, true, 4);
  ...
  void HYPERBOLICOperator<dim, fe_degree_rho, fe_degree_T, fe_degree_u,
                          n_q_points_1d_rho, n_q_points_1d_T, n_q_points_1d_u, Vec, Number>::
  vmult_rhs_rho_projection_Q0(Vec& dst, const std::vector<Vec>& src) const {
    for(unsigned int d = 0; d < src.size(); ++d)
      src[d].update_ghost_values();
  ...
  protected:
    ...
    LinearAlgebra::distributed::Vector<double> rho_contrib1;
    LinearAlgebra::distributed::Vector<double> rho_contrib2;
    LinearAlgebra::distributed::Vector<double> rho_curr;
    LinearAlgebra::distributed::Vector<double> rhs_rho;
    LinearAlgebra::distributed::Vector<double> rhs_rho_contrib1;
    LinearAlgebra::distributed::Vector<double> rhs_rho_contrib2;
    LinearAlgebra::distributed::Vector<double> rho_tmp;
    LinearAlgebra::distributed::Vector<double> rho_tmp_contrib1;
    LinearAlgebra::distributed::Vector<double> rho_tmp_contrib2;
    ...
    LinearAlgebra::distributed::Vector<double> rho_contrib1_Q0;
    LinearAlgebra::distributed::Vector<double> rho_contrib2_Q0;
    LinearAlgebra::distributed::Vector<double> rho_curr_Q0;
    LinearAlgebra::distributed::Vector<double> rhs_rho_Q0;
    LinearAlgebra::distributed::Vector<double> rhs_rho_Q0_contrib1;
    LinearAlgebra::distributed::Vector<double> rhs_rho_Q0_contrib2;

    LinearAlgebra::distributed::Vector<double> u_old_Q0;
    LinearAlgebra::distributed::Vector<double> u_tmp_2_Q0;

    LinearAlgebra::distributed::Vector<double> pres_old_Q0;
    LinearAlgebra::distributed::Vector<double> pres_tmp_2_Q0;
  ...
  void NavierStokesProjection<dim>::setup_dofs() {
    ...
    matrix_free_storage->initialize_dof_vector(rho_contrib1, 2);
    matrix_free_storage->initialize_dof_vector(rho_contrib2, 2);
    matrix_free_storage->initialize_dof_vector(rho_curr, 2);
    matrix_free_storage->initialize_dof_vector(rhs_rho, 2);
    matrix_free_storage->initialize_dof_vector(rhs_rho_contrib1, 2);
    matrix_free_storage->initialize_dof_vector(rhs_rho_contrib2, 2);
    matrix_free_storage->initialize_dof_vector(rho_tmp, 2);
    matrix_free_storage->initialize_dof_vector(rho_tmp_contrib1, 2);
    matrix_free_storage->initialize_dof_vector(rho_tmp_contrib2, 2);

    matrix_free_storage->initialize_dof_vector(rho_old_Q0, 3);
    matrix_free_storage->initialize_dof_vector(rho_tmp_2_Q0, 3);
    matrix_free_storage->initialize_dof_vector(rho_contrib1_Q0, 3);
    matrix_free_storage->initialize_dof_vector(rho_contrib2_Q0, 3);
    matrix_free_storage->initialize_dof_vector(rho_curr_Q0, 3);
    matrix_free_storage->initialize_dof_vector(rhs_rho_Q0, 3);
    matrix_free_storage->initialize_dof_vector(rhs_rho_Q0_contrib1, 3);
    matrix_free_storage->initialize_dof_vector(rhs_rho_Q0_contrib2, 3);

    matrix_free_storage->initialize_dof_vector(u_old_Q0, 4);
    matrix_free_storage->initialize_dof_vector(u_tmp_2_Q0, 4);

    matrix_free_storage->initialize_dof_vector(pres_old_Q0, 3);
    matrix_free_storage->initialize_dof_vector(pres_tmp_2_Q0, 3);
  ...
  void NavierStokesProjection<dim>::initialize() {
    ...
    VectorTools::interpolate(dof_handler_density_Q0, rho_init, rho_old_Q0);
    VectorTools::interpolate(dof_handler_velocity_Q0, u_init, u_old_Q0);
    VectorTools::interpolate(dof_handler_density_Q0, pres_init, pres_old_Q0);
  }
  ...
  void NavierStokesProjection<dim>::update_density() {
    ...
    const std::vector<unsigned int> tmp_Q0 = {3};

    if(HYPERBOLIC_stage == 1) {
      euler_matrix.initialize(matrix_free_storage, tmp, tmp);
      euler_matrix.vmult_rhs_rho_projection(rhs_rho, {rho_old, u_old});
      ...
      euler_matrix.vmult_rhs_rho_projection_Q0(rhs_rho_Q0, {rho_old_Q0, u_old_Q0, pres_old_Q0});
      euler_matrix.set_NS_stage(4);
      SolverControl solver_control_Q0(vel_max_its, vel_eps*rhs_rho_Q0.l2_norm());
      SolverCG<LinearAlgebra::distributed::Vector<double>> cg_Q0(solver_control_Q0);
      ...
      euler_matrix.set_Density_Contribution_stage(1);

      euler_matrix.initialize(matrix_free_storage, tmp, tmp);
      euler_matrix.vmult_rhs_rho_projection(rhs_rho_contrib1, {rho_old, u_old});
      euler_matrix.set_NS_stage(1);
      SolverControl solver_control_contrib1(vel_max_its, vel_eps*rhs_rho_contrib1.l2_norm());
      SolverCG<LinearAlgebra::distributed::Vector<double>> cg_contrib1(solver_control_contrib1);
      rho_contrib1.equ(1.0, rho_old);
      cg_contrib1.solve(euler_matrix, rho_contrib1, rhs_rho_contrib1, PreconditionIdentity());

      euler_matrix.initialize(matrix_free_storage, tmp_Q0, tmp_Q0);
      euler_matrix.vmult_rhs_rho_projection_Q0(rhs_rho_Q0_contrib1, {rho_old_Q0, u_old_Q0, pres_old_Q0});
      euler_matrix.set_NS_stage(4);
      SolverControl solver_control_contrib1_Q0(vel_max_its, vel_eps*rhs_rho_Q0_contrib1.l2_norm());
      SolverCG<LinearAlgebra::distributed::Vector<double>> cg_contrib1_Q0(solver_control_contrib1_Q0);
      rho_contrib1_Q0.equ(1.0, rho_old_Q0);
      cg_contrib1_Q0.solve(euler_matrix, rho_contrib1_Q0, rhs_rho_Q0_contrib1, PreconditionIdentity());
      rho_curr_Q0.equ(a31, rho_contrib1_Q0);
      FETools::interpolate(dof_handler_density_Q0, rho_contrib1_Q0, dof_handler_density, rho_tmp_contrib1);
      /*rho_contrib1.add(-1.0, rho_tmp_contrib1);
      rho_contrib1 /= (5.0*GridTools::minimal_cell_diameter(triangulation)/std::sqrt(dim)*dt);
      for(unsigned int i = 0; i < rho_contrib1.local_size(); ++i)
        rho_contrib1.local_element(i) *= (std::abs(rho_contrib1.local_element(i)) <= 1.0);
      rho_contrib1 *= (5.0*GridTools::minimal_cell_diameter(triangulation)/std::sqrt(dim)*dt);
      rho_contrib1.add(1.0, rho_tmp_contrib1);*/
      rho_curr.equ(a31, rho_contrib1);

      euler_matrix.set_Density_Contribution_stage(2);

      euler_matrix.initialize(matrix_free_storage, tmp, tmp);
      euler_matrix.vmult_rhs_rho_projection(rhs_rho_contrib2, {rho_tmp_2, u_tmp_2, rho_old});
      euler_matrix.set_NS_stage(1);
      SolverControl solver_control_contrib2(vel_max_its, vel_eps*rhs_rho_contrib2.l2_norm());
      SolverCG<LinearAlgebra::distributed::Vector<double>> cg_contrib2(solver_control_contrib2);
      rho_contrib2.equ(1.0, rho_old);
      cg_contrib2.solve(euler_matrix, rho_contrib2, rhs_rho_contrib2, PreconditionIdentity());

      euler_matrix.initialize(matrix_free_storage, tmp_Q0, tmp_Q0);
      euler_matrix.vmult_rhs_rho_projection_Q0(rhs_rho_Q0_contrib2, {rho_tmp_2_Q0, u_tmp_2_Q0, pres_tmp_2_Q0, rho_old_Q0});
      euler_matrix.set_NS_stage(4);
      SolverControl solver_control_contrib2_Q0(vel_max_its, vel_eps*rhs_rho_Q0_contrib2.l2_norm());
      SolverCG<LinearAlgebra::distributed::Vector<double>> cg_contrib2_Q0(solver_control_contrib2_Q0);
      rho_contrib2_Q0.equ(1.0, rho_old_Q0);
      cg_contrib2_Q0.solve(euler_matrix, rho_contrib2_Q0, rhs_rho_Q0_contrib2, PreconditionIdentity());
      rho_curr_Q0.add(a32, rho_contrib2_Q0);
      FETools::interpolate(dof_handler_density_Q0, rho_contrib2_Q0, dof_handler_density, rho_tmp_contrib2);
      /*rho_contrib2.add(-1.0, rho_tmp_contrib2);
      rho_contrib2 /= (5.0*GridTools::minimal_cell_diameter(triangulation)/std::sqrt(dim)*dt);
      for(unsigned int i = 0; i < rho_contrib1.local_size(); ++i)
        rho_contrib2.local_element(i) *= (std::abs(rho_contrib2.local_element(i)) <= 1.0);
      rho_contrib2 *= (5.0*GridTools::minimal_cell_diameter(triangulation)/std::sqrt(dim)*dt);
      rho_contrib2.add(1.0, rho_tmp_contrib2);*/
      rho_curr.add(a32, rho_contrib2);
    }
  }
  ...
  void NavierStokesProjection<dim>::output_results(const unsigned int step) {
    ...
    std::vector<std::string> velocity_names_Q0(dim, "u_Q0");
    std::vector<DataComponentInterpretation::DataComponentInterpretation>
    component_interpretation_velocity_Q0(dim, DataComponentInterpretation::component_is_part_of_vector);
    u_old_Q0.update_ghost_values();
    data_out.add_data_vector(dof_handler_velocity_Q0, u_old_Q0, velocity_names_Q0, component_interpretation_velocity_Q0);
    pres_old_Q0.update_ghost_values();
    data_out.add_data_vector(dof_handler_density_Q0, pres_old_Q0, "p_Q0", {DataComponentInterpretation::component_is_scalar});
  ...
  void NavierStokesProjection<dim>::run(const bool verbose, const unsigned int output_interval) {
    ...
    while(std::abs(T - time) > 1e-10) {
      ...
      FETools::interpolate(dof_handler_velocity, u_tmp_2, dof_handler_velocity_Q0, u_tmp_2_Q0);
      FETools::interpolate(dof_handler_density, pres_tmp_2, dof_handler_density_Q0, pres_tmp_2_Q0);
      update_density();
      pcout<<"Minimal density "<<get_minimal_density()<<std::endl;
      pcout<<"Maximal density "<<get_maximal_density()<<std::endl;
      ...
      FETools::interpolate(dof_handler_density, pres_old, dof_handler_density_Q0, pres_old_Q0);
      u_old.equ(1.0, u_curr);
      FETools::interpolate(dof_handler_velocity, u_old, dof_handler_velocity_Q0, u_old_Q0);
