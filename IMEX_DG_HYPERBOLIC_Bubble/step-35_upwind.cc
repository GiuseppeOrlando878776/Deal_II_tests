...
namespace Step35 {
  ...
  namespace EquationData {
    ...
    double Pressure<dim>::value(const Point<dim>&  p, const unsigned int component) const {
      ...
      double theta = A*(r <= a) + A*std::exp(-(r-a)*(r-a)/(s*s))*(r > a);
      theta += 298.15;

      const double T = std::pow(theta, EquationData::Cp_Cv)/std::pow(101325.0/(1.225*287.05), EquationData::Cp_Cv - 1.0);

      return 1.225*287.05*T;
    }
    ...
    if(HYPERBOLIC_stage == 1) {
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& rho_old_D        = phi_rho_old.get_value(q);
          const auto& pres_old_D       = phi_pres_old.get_value(q);
          const auto& u_old_D          = phi_u_old.get_value(q) - 2.0*scalar_product(phi_u_old.get_value(q), n_plus)*n_plus;
          ...
          const auto& rho_tmp_2        = phi_rho_tmp_2.get_value(q);
          const auto& u_fixed          = phi_u_fixed.get_value(q);
          const auto& rho_tmp_2_D      = rho_tmp_2;
          ...
          const auto& jump_rhok_fixed  = rho_tmp_2*0.5*Ma*Ma*scalar_product(u_fixed, u_fixed) -
                                         rho_tmp_2_D*0.5*Ma*Ma*scalar_product(u_fixed_D, u_fixed_D);
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& u_old_D          = phi_u_old.get_value(q) - 2.0*scalar_product(phi_u_old.get_value(q), n_plus)*n_plus;
          const auto& rho_tmp_2_D      = phi_rho_tmp_2.get_value(q);
          const auto& pres_tmp_2_D     = phi_pres_tmp_2.get_value(q);
          const auto& u_tmp_2_D        = phi_u_tmp_2.get_value(q) - 2.0*scalar_product(phi_u_tmp_2.get_value(q), n_plus)*n_plus;
          ...
          const auto& rho_curr           = phi_rho_curr.get_value(q);
          const auto& u_fixed            = phi_u_fixed.get_value(q);
          const auto& rho_curr_D         = rho_curr;
          const auto& u_fixed_D          = u_fixed - 2.0*scalar_product(u_fixed, n_plus)*n_plus;
          const auto& lambda_fixed       = std::max(scalar_product(u_fixed, u_fixed),
                                                    scalar_product(u_fixed_D, u_fixed_D));
          const auto& jump_rhok_fixed    = rho_curr*0.5*Ma*Ma*scalar_product(u_fixed, u_fixed) -
                                           rho_curr_D*0.5*Ma*Ma*scalar_product(u_fixed_D, u_fixed_D);
          ...
          phi.submit_value(-a31*dt*Ma*Ma*scalar_product(avg_kinetic_old, n_plus)
                           ...
                           -a32_tilde*dt*scalar_product(avg_enthalpy_tmp_2, n_plus)
                           -a32*dt*0.5*lambda_tmp_2*jump_rhoE_tmp_2
                           -a33*dt*0.5*lambda_fixed*jump_rhok_fixed, q);
        }
        phi.integrate_scatter(true, false, dst);
      }
    }
  ...
  assemble_boundary_term_enthalpy(const MatrixFree<dim, Number>&               data,
    ...
    for(unsigned int face = face_range.first; face < face_range.second; ++face) {
      ...
      for(unsigned int q = 0; q < phi.n_q_points; ++q) {
        ...
        const auto& pres_fixed        = phi_pres_fixed.get_value(q);
        auto        u_D               = phi_src.get_value(q) - 2.0*scalar_product(phi_src.get_value(q), n_plus)*n_plus;
