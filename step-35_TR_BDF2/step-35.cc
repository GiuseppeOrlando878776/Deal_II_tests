...
namespace Step35
{
  ...
  namespace RunTimeParameters
  {
    ...
    public:
      ...
      double theta; //--- Parameter for TR-BDF2 time scheme
    ...
    void Data_Storage::read_data(const std::string &filename)
    {
      ...
      prm.enter_subsection("Time step data");
      {
        ...
        theta = prm.get_double("theta"); //--- Read TR-BDF2 parameter
      }
      prm.leave_subsection();
  ...
  protected:
    ...
    const double       dt;
    const double       theta; //--- TR-BDF2 parameter
    int                TR_BDF2_stage; //--- Flag to check at which current stage of TR-BDF2 are
    const double       t_0;
    const double       T;
    const double       Re;
  ...
  private:
    ...
    struct InitGradScratchData {
      ...
      InitGradScratchData(const FE_Q<dim>& fe_v, const FE_Q<dim>& fe_p,
    ...
    //--- We add an extra variable to store the explicit advective part
    //--- which has to be added to the right-hand side
    struct AdvectionPerTaskData {
      FullMatrix<double>                   local_advection;
      std::array<Vector<double>, dim> 	   local_vel_rhs; //--- Extra variable for rhs
    ...
    struct AdvectionScratchData {
      ...
      std::vector<Point<dim>>     u_n_local; //--- Extra variable to store u_n or u_(n+theta)
      std::vector<Tensor<1, dim>> grad_u_star;
      std::vector<Tensor<1, dim>> grad_u_n; //--- Extra variable to store nabla(u_n) or nabla(u_(n+theta))
      std::vector<double>         u_star_tmp;
      std::vector<double>         u_n_tmp; //--- Extra variable to store u_n or u_(n+theta)
      FEValues<dim>               fe_val;
      AdvectionScratchData(const FE_Q<dim>&  fe, const QGauss<dim>& quad,
                           const UpdateFlags flags): nqp(quad.size()),
      ...
      AdvectionScratchData(const AdvectionScratchData &data): nqp(data.nqp),
                                                              dpc(data.dpc),
                                                              u_star_local(nqp),
                                                              u_n_local(nqp),
                                                              grad_u_star(nqp),
  ...
  NavierStokesProjection<dim>::NavierStokesProjection(const RunTimeParameters::Data_Storage &data):
      ...
      dt(data.dt),
      theta(data.theta), //--- Save in the NavierStokes class the TR-BDF2 parameter value
      ...
      vel_diag_strength(data.vel_diag_strength) {
        ...
        TR_BDF2_stage = 1; //--- Initialize flag of TR-BDF2 stage
  ...
  void NavierStokesProjection<dim>::initialize() {
    //--- Set the constant contribution stiffness matrix stage 1 TR-BDF2
    vel_Laplace_plus_Mass_stage_1 = 0.;
    vel_Laplace_plus_Mass_stage_1.add(0.5/Re, vel_Laplace);
    vel_Laplace_plus_Mass_stage_1.add(1.0/(theta*dt), vel_Mass);

    //--- Set the constant contribution stiffness matrix stage 2 TR-BDF2
    vel_Laplace_plus_Mass_stage_2 = 0.;
    vel_Laplace_plus_Mass_stage_2.add(1.0/(2.0 - theta), vel_Laplace);
    vel_Laplace_plus_Mass_stage_2.add(1.0/((1.0 - theta)*dt), vel_Mass);
  ...
  void NavierStokesProjection<dim>::initialize_velocity_matrices() {
    ...
    vel_Laplace_plus_Mass_stage_1.reinit(sparsity_pattern_velocity); //--- Initialize first stage matrix variable
    vel_Laplace_plus_Mass_stage_2.reinit(sparsity_pattern_velocity); //--- Initialize second stage matrix variable
    for (unsigned int d = 0; d < dim; ++d)
  ...
  void NavierStokesProjection<dim>::diffusion_step(const bool reinit_prec) {
    pres_tmp.equ(-1., pres_n); //--- Save however in pres_tmp in order to be coherent for assemble_advection_term

    assemble_advection_term();

    //--- Now we have to assemble the remaining part of known term and the bcs for each step
    //--- (this is a reason why we precomputed mass and laplace matrix)
    //--- In the previous version the only contribution was related to the time derivative approximation through BDF2 scheme
    //--- whereas we have less terms from the time derivative but other terms from the "spatial" part.
    ...
    if(TR_BDF2_stage == 1) {
      for(unsigned int d = 0; d < dim; ++d) {
        ...
        vel_it_matrix[d].copy_from(vel_Laplace_plus_Mass_stage_1); //--- Copy the right constant contribution stiffness matrix
        vel_it_matrix[d].add(1.0, vel_Advection);
    ...
    //--- Let us focus now on the second stage
      ...
      for(unsigned int d = 0; d < dim; ++d) {
        ...
        vel_it_matrix[d].copy_from(vel_Laplace_plus_Mass_stage_2); //--- Copy the right constant contribution stiffness matrix
        vel_it_matrix[d].add(1.0, vel_Advection);
        u_n_minus_1[d] = u_n[d];
  ...
  void NavierStokesProjection<dim>::diffusion_component_solve(const unsigned int d) {
    ...
    if(TR_BDF2_stage == 1)
      gmres.solve(vel_it_matrix[d], u_n_minus_1[d], force[d], prec_velocity[d]);
    else
  ...
  void NavierStokesProjection<dim>::assemble_advection_term() {
    vel_Advection = 0.0;
    for(unsigned int d = 0; d < dim; ++d)
      force[d] = 0.0; //--- Initialize to zero the rhs
  ...
  template <int dim>
  void NavierStokesProjection<dim>::assemble_one_cell_of_advection(const typename DoFHandler<dim>::active_cell_iterator &cell,
                                                                   AdvectionScratchData& scratch,
                                                                   AdvectionPerTaskData& data) {
    ...
    data.local_advection = 0.0;
    if(TR_BDF2_stage == 1) {
      for(unsigned int q = 0; q < scratch.nqp; ++q)
        for(unsigned int i = 0; i < scratch.dpc; ++i)
          for(unsigned int j = 0; j < scratch.dpc; ++j)
            data.local_advection(i, j) += 0.5*(scratch.u_star_local[q]*
      ...
      for(unsigned int q = 0; q < scratch.nqp; ++q)
        for(unsigned int i = 0; i < scratch.dpc; ++i)
          for(unsigned int j = 0; j < scratch.dpc; ++j)
            data.local_advection(i, j) += 1.0/(2.0 - theta)*(scratch.u_star_local[q]*
                                                             scratch.fe_val.shape_grad(j, q)*
                                                             scratch.fe_val.shape_value(i, q)
                                                           + scratch.u_star_tmp[q] *
                                                             scratch.fe_val.shape_value(i, q) *
    ...
    //--- Now we are left to assemble the explicit part of the advective part
    //--- which contributes to the rhs. Let us start with the first stage
    if(TR_BDF2_stage == 1) {
      for(int d = 0; d < dim; ++d) {
        data.local_vel_rhs[d] = 0.0;
        scratch.fe_val.get_function_values(u_n[d], scratch.u_n_tmp);
        scratch.fe_val.get_function_gradients(u_n[d], scratch.grad_u_n);
        for(unsigned int q = 0; q < scratch.nqp; ++q)
          for(unsigned int i = 0; i < scratch.dpc; ++i)
              data.local_vel_rhs[d](i) += 0.5*(scratch.u_star_local[q]*scratch.grad_u_n[q] +
                                               scratch.u_star_tmp[q]*scratch.u_n_tmp[q])*
                                               scratch.fe_val.shape_value(i, q)*
                                               scratch.fe_val.JxW(q);
      }
    }
    //--- Let us pass to the second stage
    else {
      //--- Compute div(u_n)
      for(unsigned int d = 0; d < dim; ++d) {
        scratch.fe_val.get_function_gradients(u_n[d], scratch.grad_u_n);
        for(unsigned int q = 0; q < scratch.nqp; ++q) {
          if(d == 0)
            scratch.u_star_tmp[q] = 0.;
          scratch.u_star_tmp[q] += scratch.grad_u_n[q][d];
        }
      }
      //--- Add the contribution due to u_n
      for(unsigned int d = 0; d < dim; ++d) {
        scratch.fe_val.get_function_values(u_n[d], scratch.u_n_tmp);
        for(unsigned int q = 0; q < scratch.nqp; ++q)
          scratch.u_n_local[q](d) = scratch.u_n_tmp[q];
      }
      for(unsigned int d = 0; d < dim; ++d) {
        data.local_vel_rhs[d] = 0.0;
        for(unsigned int q = 0; q < scratch.nqp; ++q)
          for(unsigned int i = 0; i < scratch.dpc; ++i)
              data.local_vel_rhs[d](i) += (1.0 - theta)/(2.0*(2.0 - theta))*
                                          (scratch.u_n_local[q]*scratch.grad_u_n[q] +
                                           scratch.u_star_tmp[q]*scratch.u_n_tmp[q])*
                                           scratch.fe_val.shape_value(i, q)*
                                           scratch.fe_val.JxW(q);
      }
      //--- Compute the divergence of u_(n+theta) stored in u_(n-1)
      for(unsigned int d = 0; d < dim; ++d) {
        scratch.fe_val.get_function_gradients(u_n_minus_1[d], scratch.grad_u_n);
        for(unsigned int q = 0; q < scratch.nqp; ++q) {
          if(d == 0)
            scratch.u_star_tmp[q] = 0.;
          scratch.u_star_tmp[q] += scratch.grad_u_n[q][d];
      ...
      //--- Add the contribution due to u_(n+theta) stored in u_(n-1)
      for(unsigned int d = 0; d < dim; ++d) {
        scratch.fe_val.get_function_values(u_n_minus_1[d], scratch.u_n_tmp);
        for(unsigned int q = 0; q < scratch.nqp; ++q)
          scratch.u_n_local[q](d) = scratch.u_n_tmp[q];
      }
      for(int d = 0; d < dim; ++d) {
  ...
  template <int dim>
  void NavierStokesProjection<dim>::copy_advection_local_to_global(const AdvectionPerTaskData &data) {
  ...
  void NavierStokesProjection<dim>::update_pressure(const bool reinit_prec) {
    switch(type) {
        ...
        case RunTimeParameters::Method::rotational:
          ...
          pres_n_minus_1 = pres_tmp;
          prec_mass.solve(pres_n_minus_1);
  ...
  void NavierStokesProjection<dim>::output_results(const unsigned int step) {
    ...
    Assert(joint_dof_handler.n_dofs() == ((dim + 1) * dof_handler_velocity.n_dofs() +
                                          dof_handler_pressure.n_dofs()),
           ExcInternalError());
      ...
      for(unsigned int i = 0; i < joint_fe.n_dofs_per_cell(); ++i)
        switch(joint_fe.system_to_base_index(i).first.first) {
          case 0:
            ...
            u_n[joint_fe.system_to_base_index(i).first.second](loc_vel_dof_indices[joint_fe.system_to_base_index(i).second]);
            break;
          case 1:
          ...
          case 2:
            Assert(joint_fe.system_to_base_index(i).first.second == 0,
                   ExcInternalError());
            joint_solution(loc_joint_dof_indices[i]) = rot_u(loc_vel_dof_indices[joint_fe.system_to_base_index(i).second]);
            break;
          default:
