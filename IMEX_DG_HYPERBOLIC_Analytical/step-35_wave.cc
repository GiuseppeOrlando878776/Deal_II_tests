...
namespace Step35 {
  ...
  namespace EquationData {
    ...
    // directly from the deal.II built-in class Function
    template<int dim>
    class Pressure_Init: public Function<dim> {
    public:
      Pressure_Init(const double initial_time = 0.0);
    ...
    template<int dim>
    Pressure_Init<dim>::Pressure_Init(const double initial_time): Function<dim>(1, initial_time) {}


    template<int dim>
    double Pressure_Init<dim>::value(const Point<dim>&  p, const unsigned int component) const {
      (void)component;
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi_p.n_q_points; ++q) {
          ...
          const auto  lambda_tmp_2   = std::max(std::sqrt(scalar_product(phi_u_tmp_2_p.get_value(q), phi_u_tmp_2_p.get_value(q))) +
                                                ...
                                                1.0/Ma*std::sqrt(EquationData::Cp_Cv*
  ...
  private:
    ...
    EquationData::Pressure_Init<dim> pres_init;
