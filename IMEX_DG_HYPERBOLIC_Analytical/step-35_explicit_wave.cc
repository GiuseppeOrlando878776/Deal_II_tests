...
namespace Step35 {
  ...
  namespace EquationData {
    ...
    double Energy<dim>::value(const Point<dim>&  p, const unsigned int component) const {
      ...
      return 10.0/(EquationData::Cp_Cv - 1.0);
    }
    ...
    // directly from the deal.II built-in class Function
    template<int dim>
    class Energy_Init: public Function<dim> {
    public:
      Energy_Init(const double initial_time = 0.0);
    ...
    template<int dim>
    Energy_Init<dim>::Energy_Init(const double initial_time): Function<dim>(1, initial_time) {}


    template<int dim>
    double Energy_Init<dim>::value(const Point<dim>&  p, const unsigned int component) const {
      (void)component;

      Point<dim> x0;
      x0[0] = 5.;
      return (10.0 + 0.1*std::exp(-((p[0] - x0[0])*(p[0] - x0[0])/2.0 + (p[1] - x0[1])*(p[1] - x0[1])/2.0)))/
  ...
  public:
    ...
    void advance_E_boundary_time(const double time_advance);
    ...
    Number compute_min_pressure(const std::vector<Vec>& src) const;

    Number compute_max_pressure(const std::vector<Vec>& src) const;
  ...
  private:
    ...
    EquationData::Energy<dim>   E_boundary;
  ...
  HYPERBOLICOperator(RunTimeParameters::Data_Storage& data): MatrixFreeOperators::Base<dim, Vec>(),
                                                             ...
                                                             u_boundary(data.initial_time), E_boundary(data.initial_time) {}
  ...
  // Advance energy boundary time
  //
  template<int dim, int fe_degree_rho, int fe_degree_T, int fe_degree_u,
           int n_q_points_1d_rho, int n_q_points_1d_T, int n_q_points_1d_u, typename Vec, typename Number>
  void HYPERBOLICOperator<dim, fe_degree_rho, fe_degree_T, fe_degree_u,
                          n_q_points_1d_rho, n_q_points_1d_T, n_q_points_1d_u, Vec, Number>::
  advance_E_boundary_time(const double time_advance) {
    E_boundary.advance_time(time_advance);
  }
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi_p.n_q_points; ++q) {
          ...
          const auto  lambda     = std::max(std::sqrt(scalar_product(phi_u_old_p.get_value(q)/phi_rho_old_p.get_value(q),
                                            ...
                                            1.0/Ma*
                                            std::sqrt(EquationData::Cp_Cv*pres_old_p/phi_rho_old_p.get_value(q)),
                                            ...
                                            1.0/Ma*
                                            std::sqrt(EquationData::Cp_Cv*pres_old_m/phi_rho_old_m.get_value(q)));
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi_p.n_q_points; ++q) {
          ...
          const auto  lambda_tmp_2   = std::max(std::sqrt(scalar_product(phi_u_tmp_2_p.get_value(q)/phi_rho_tmp_2_p.get_value(q),
                                                ...
                                                1.0/Ma*
                                                std::sqrt(EquationData::Cp_Cv*pres_tmp_2_p/phi_rho_tmp_2_p.get_value(q)),
                                                ...
                                                1.0/Ma*
                                                std::sqrt(EquationData::Cp_Cv*pres_tmp_2_m/phi_rho_tmp_2_m.get_value(q)));
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto  lambda   = std::max(std::sqrt(scalar_product(phi_u_old.get_value(q)/phi_rho_old.get_value(q),
                                          ...
                                          1.0/Ma*
                                          std::sqrt(EquationData::Cp_Cv*pres_old/phi_rho_old.get_value(q)),
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          for(unsigned int v = 0; v < VectorizedArray<Number>::size(); ++v) {
            ...
            for(unsigned int d = 0; d < dim; ++d) {
              u_old_D[d][v]   = u_boundary.value(point, d);
              u_tmp_2_D[d][v] = u_boudary_tmp_2.value(point, d);
            }
          }
          ...
          const auto  lambda_tmp_2   = std::max(std::sqrt(scalar_product(phi_u_tmp_2.get_value(q)/phi_rho_tmp_2.get_value(q),
                                                ...
                                                1.0/Ma*
                                                std::sqrt(EquationData::Cp_Cv*pres_tmp_2/phi_rho_tmp_2.get_value(q)),
    ...
    if(HYPERBOLIC_stage == 1) {
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          phi.submit_value(phi_u_old.get_value(q) - 0.0*a21*dt/(Fr*Fr)*phi_rho_old.get_value(q)*e_k, q);
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          phi.submit_value(phi_u_old.get_value(q) -
                           0.0*a31*dt/(Fr*Fr)*phi_rho_old.get_value(q)*e_k -
                           0.0*a32*dt/(Fr*Fr)*phi_rho_tmp_2.get_value(q)*e_k, q);
    ...
    if(HYPERBOLIC_stage == 1) {
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          phi.submit_value(E_old - 0.0*a21*dt*Ma*Ma/(Fr*Fr)*u_old[dim - 1], q);
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          phi.submit_value(E_old - 0.0*a31*dt*Ma*Ma/(Fr*Fr)*u_old[dim - 1] - 0.0*a32*dt*Ma*Ma/(Fr*Fr)*u_tmp_2[dim - 1], q);
          phi.submit_gradient(a31*dt*(Ma*Ma*0.5*scalar_product(u_old/rho_old, u_old/rho_old)*u_old +
                              ...
                              a32*dt*(Ma*Ma*0.5*scalar_product(u_tmp_2/rho_tmp_2, u_tmp_2/rho_tmp_2)*u_tmp_2 +
                                      (E_tmp_2 - 0.5*Ma*Ma*scalar_product(u_tmp_2, u_tmp_2)/rho_tmp_2 + pres_tmp_2)*u_tmp_2/rho_tmp_2), q);
        }
        phi.integrate_scatter(true, true, dst);
      }
    }
    ...
    if(HYPERBOLIC_stage == 1) {
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          auto        rho_old_D        = VectorizedArray<Number>();
          auto        pres_old_D       = VectorizedArray<Number>();
          auto        E_old_D          = VectorizedArray<Number>();
          auto        u_old_D          = Tensor<1, dim, VectorizedArray<Number>>();
          for(unsigned int v = 0; v < VectorizedArray<Number>::size(); ++v) {
            Point<dim> point;
            for(unsigned int d = 0; d < dim; ++d)
            ...
            pres_old_D[v] = pres_boundary.value(point);
            rho_old_D[v]  = rho_boundary.value(point);
            E_old_D[v]    = E_boundary.value(point);
            for(unsigned int d = 0; d < dim; ++d)
              u_old_D[d][v] = u_boundary.value(point, d);
          }
      ...
      auto rho_boundary_tmp_2  = rho_boundary;
      auto u_boudary_tmp_2     = u_boundary;
      auto E_boundary_tmp_2    = E_boundary;
      pres_boundary_tmp_2.advance_time(gamma*dt);
      rho_boundary_tmp_2.advance_time(gamma*dt);
      u_boudary_tmp_2.advance_time(gamma*dt);
      E_boundary_tmp_2.advance_time(gamma*dt);
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          auto        rho_tmp_2_D      = VectorizedArray<Number>();
          auto        pres_tmp_2_D     = VectorizedArray<Number>();
          auto        E_tmp_2_D          = VectorizedArray<Number>();
          auto        u_tmp_2_D        = Tensor<1, dim, VectorizedArray<Number>>();
          for(unsigned int v = 0; v < VectorizedArray<Number>::size(); ++v) {
            Point<dim> point;
            for(unsigned int d = 0; d < dim; ++d)
            ...
            rho_tmp_2_D[v]  = rho_boundary_tmp_2.value(point);
            E_old_D[v]      = E_boundary.value(point);
            E_tmp_2_D[v]    = E_boundary_tmp_2.value(point);
            for(unsigned int d = 0; d < dim; ++d) {
              u_old_D[d][v]   = u_boundary.value(point, d);
              u_tmp_2_D[d][v] = u_boudary_tmp_2.value(point, d);
            }
  ...
  compute_max_celerity(const std::vector<Vec>& src) const {
    ...
    FEEvaluation<dim, fe_degree_T, n_q_points_1d_T, 1, Number> phi_E(*(this->data), 1);
    ...
    for(unsigned int cell = 0; cell < this->data->n_cell_batches(); ++cell) {
      ...
      for(unsigned int q = 0; q < phi_u.n_q_points; ++q) {
        ...
        const auto& E    = phi_E.get_value(q);
        const auto& pres = (E - 0.5/rho*scalar_product(u,u))*(EquationData::Cp_Cv - 1.0);
  ...
  Number HYPERBOLICOperator<dim, fe_degree_rho, fe_degree_T, fe_degree_u,
                            n_q_points_1d_rho, n_q_points_1d_T, n_q_points_1d_u, Vec, Number>::
  compute_min_pressure(const std::vector<Vec>& src) const {
    for(unsigned int d = 0; d < src.size(); ++d)
      src[d].update_ghost_values();
    ...
    Number min_pressure = std::numeric_limits<Number>::max();
    ...
    for(unsigned int cell = 0; cell < this->data->n_cell_batches(); ++cell) {
      ...
      VectorizedArray<Number> local_min = std::numeric_limits<Number>::max();
      ...
      for(unsigned int q = 0; q < phi_u.n_q_points; ++q) {
        ...
        local_min = std::min(local_min, pres);
      }
      for(unsigned int v = 0; v < this->data->n_active_entries_per_cell_batch(cell); ++v)
        min_pressure = std::min(min_pressure, local_min[v]);
    }

    min_pressure = Utilities::MPI::min(min_pressure, MPI_COMM_WORLD);
  ...
  Number HYPERBOLICOperator<dim, fe_degree_rho, fe_degree_T, fe_degree_u,
                            n_q_points_1d_rho, n_q_points_1d_T, n_q_points_1d_u, Vec, Number>::
  compute_max_pressure(const std::vector<Vec>& src) const {
    for(unsigned int d = 0; d < src.size(); ++d)
      src[d].update_ghost_values();
    ...
    Number max_pressure = std::numeric_limits<Number>::min();
    ...
    for(unsigned int cell = 0; cell < this->data->n_cell_batches(); ++cell) {
      ...
      VectorizedArray<Number> local_max = std::numeric_limits<Number>::min();
      ...
      for(unsigned int q = 0; q < phi_u.n_q_points; ++q) {
        ...
        local_max = std::max(local_max, pres);
      }
      for(unsigned int v = 0; v < this->data->n_active_entries_per_cell_batch(cell); ++v)
        max_pressure = std::max(max_pressure, local_max[v]);
    }

    max_pressure = Utilities::MPI::max(max_pressure, MPI_COMM_WORLD);
  ...
  private:
    ...
    EquationData::Energy_Init<dim> E_init;
    ...
    HYPERBOLICOperator<dim, EquationData::degree_rho, EquationData::degree_T, EquationData::degree_u,
                            EquationData::degree_rho + 2, EquationData::degree_T + 2,EquationData::degree_u + 2,
  ...
  void NavierStokesProjection<dim>::setup_dofs() {
    ...
    quadratures.push_back(QGauss<1>(EquationData::degree_u + 2));
    quadratures.push_back(QGauss<1>(EquationData::degree_T + 2));
    quadratures.push_back(QGauss<1>(EquationData::degree_rho + 2));
  ...
  void NavierStokesProjection<dim>::update_density() {
    ...
    if(HYPERBOLIC_stage == 1)
      navier_stokes_matrix.vmult_rhs_rho_projection(rhs_rho, {rho_old, u_old, E_old});
    else
      navier_stokes_matrix.vmult_rhs_rho_projection(rhs_rho, {rho_old, u_old, E_old,
  ...
  void NavierStokesProjection<dim>::update_momentum() {
    ...
    if(HYPERBOLIC_stage == 1)
      navier_stokes_matrix.vmult_rhs_momentum_update(rhs_u, {rho_old, u_old, E_old});
    else
      navier_stokes_matrix.vmult_rhs_momentum_update(rhs_u, {rho_old, u_old, E_old,
                                                             rho_tmp_2, u_tmp_2, E_tmp_2});
    ...
    if(HYPERBOLIC_stage == 1) {
      ...
      cg.solve(navier_stokes_matrix, u_tmp_2, rhs_u, PreconditionIdentity());
    }
    else {
      u_curr.equ(1.0, u_tmp_2);
      cg.solve(navier_stokes_matrix, u_curr, rhs_u, PreconditionIdentity());
    }
  }
  ...
  void NavierStokesProjection<dim>::update_energy() {
    ...
    if(HYPERBOLIC_stage == 1)
      navier_stokes_matrix.vmult_rhs_energy(rhs_E, {rho_old, u_old, E_old});
    else
      navier_stokes_matrix.vmult_rhs_energy(rhs_E, {rho_old, u_old, E_old,
                                                    rho_tmp_2, u_tmp_2, E_tmp_2});
    ...
    if(HYPERBOLIC_stage == 1) {
      ...
      cg.solve(navier_stokes_matrix, E_tmp_2, rhs_E, PreconditionIdentity());
    }
    else {
      E_curr.equ(1.0, E_tmp_2);
      cg.solve(navier_stokes_matrix, E_curr, rhs_E, PreconditionIdentity());
    }
  }
  ...
  void NavierStokesProjection<dim>::project_velocity() {
    ...
    navier_stokes_matrix.vmult_rhs_velocity_projection(rhs_u, {rho_curr, u_curr});

    SolverControl solver_control(vel_max_its, 1e-12*rhs_u.l2_norm());
    SolverCG<LinearAlgebra::distributed::Vector<double>> cg(solver_control);

    u_proj.equ(1.0, u_old);
    cg.solve(navier_stokes_matrix, u_proj, rhs_u, PreconditionIdentity());
  }
  ...
  void NavierStokesProjection<dim>::project_pressure() {
    ...
    navier_stokes_matrix.set_NS_stage(5);

    if(HYPERBOLIC_stage == 1)
      navier_stokes_matrix.vmult_rhs_pressure_projection(rhs_pres, {rho_tmp_2, u_tmp_2, E_tmp_2});
    else
      navier_stokes_matrix.vmult_rhs_pressure_projection(rhs_pres, {rho_curr, u_curr, E_curr});
    ...
    if(HYPERBOLIC_stage == 1) {
      ...
      cg.solve(navier_stokes_matrix, pres_tmp_2, rhs_pres, PreconditionIdentity());
    }
    else {
      pres_old.equ(1.0, pres_tmp_2);
      cg.solve(navier_stokes_matrix, pres_old, rhs_pres, PreconditionIdentity());
    }
  }
  ...
  void NavierStokesProjection<dim>::analyze_results() {
    ...
    pcout << "Verification via L2 error momentum:    "<< error_vel_L2 << std::endl;
    pcout << "Verification via L2 relative error momentum:    "<< error_rel_vel_L2 << std::endl;
    ...
    VectorTools::integrate_difference(dof_handler_temperature, E_old, E_exact,
                                      L2_error_per_cell_energy, quadrature_temperature, VectorTools::L2_norm);
    const double error_energy_L2 = VectorTools::compute_global_error(triangulation, L2_error_per_cell_energy, VectorTools::L2_norm);
    VectorTools::integrate_difference(dof_handler_temperature, E_tmp_2, E_exact,
                                      L2_rel_error_per_cell_energy, quadrature_temperature, VectorTools::L2_norm);
    const double L2_energy = VectorTools::compute_global_error(triangulation, L2_rel_error_per_cell_energy, VectorTools::L2_norm);
    const double error_rel_energy_L2 = error_energy_L2/L2_energy;
    pcout << "Verification via L2 error energy:    "<< error_energy_L2 << std::endl;
    pcout << "Verification via L2 relative error energy:    "<< error_rel_energy_L2 << std::endl;
    ...
    if(Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 0) {
      ...
      output_error_pres << error_energy_L2 << std::endl;
      output_error_pres << error_rel_energy_L2 << std::endl;
    }
  }
  ...
  void NavierStokesProjection<dim>::run(const bool verbose, const unsigned int output_interval) {
    ...
    while(std::abs(T - time) > 1e-10) {
      ...
      pcout<<"Minimal pressure "<<navier_stokes_matrix.compute_min_pressure({rho_tmp_2, u_tmp_2, E_tmp_2})<<std::endl;
      pcout<<"Maximal pressure "<<navier_stokes_matrix.compute_max_pressure({rho_tmp_2, u_tmp_2, E_tmp_2})<<std::endl;
      ...
      E_exact.advance_time((1.0 - gamma)*dt);
      ...
      update_energy();
      pcout<<"Minimal density "<<get_minimal_density()<<std::endl;
      pcout<<"Minimal pressure "<<navier_stokes_matrix.compute_min_pressure({rho_curr, u_curr, E_curr})<<std::endl;
      pcout<<"Maximal pressure "<<navier_stokes_matrix.compute_max_pressure({rho_curr, u_curr, E_curr})<<std::endl;
      ...
      navier_stokes_matrix.advance_E_boundary_time(dt);
      rho_old.equ(1.0, rho_curr);
      u_old.equ(1.0, u_curr);
      E_old.equ(1.0, E_curr);
      const double max_celerity = navier_stokes_matrix.compute_max_celerity({rho_old, u_old, E_old});
