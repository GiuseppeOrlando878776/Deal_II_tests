...
namespace Step35 {
  ...
  double NavierStokesProjection<dim>::get_maximal_velocity() {
    VectorTools::integrate_difference(dof_handler_velocity, HYPERBOLIC_stage == 1 ? u_tmp_2 : u_old, ZeroFunction<dim>(dim),
  ...
  void NavierStokesProjection<dim>::run(const bool verbose, const unsigned int output_interval) {
    ...
    while(std::abs(T - time) > 1e-10) {
      ...
      pcout<<"Maximal pressure "<<get_maximal_pressure()<<std::endl;
      pcout<<"Minimal pressure "<<get_minimal_pressure()<<std::endl;
      pcout<<"Maximal velocity "<<get_maximal_velocity()<<std::endl;
