...
namespace Step35 {
  ...
  namespace EquationData {
    ...
    double Pressure<dim>::value(const Point<dim>&  p, const unsigned int component) const {
      ...
      const double y0 = 1250.0;
      /*const double xr = 4000.0;
      const double yr = 2000.0;*/
      const double r  = std::sqrt((p[0] - x0)*(p[0] - x0) + (p[1] - y0)*(p[1] - y0));
      const double A  = -15.0;
      const double rb = 50.0;

      const double theta_prime = A*std::exp(-(r-rb)*(r-rb)/10000.0)*(r <= rb);
      const double theta = 303.0 + theta_prime;

      const double Cp = EquationData::Cp_Cv/(EquationData::Cp_Cv - 1.0)*287.05;
      const double pi_e = 1.0 - 9.81*p[1]/(Cp*303.0);
      const double rho_bar = 100000.0/(287.05*303.0)*std::pow(pi_e, 1.0/(EquationData::Cp_Cv - 1.0));
      const double rho = rho_bar*(1.0 - theta_prime/theta);

      return 100000.0*std::pow(287.05*rho*theta/100000.0, EquationData::Cp_Cv);
    }
  ...
  private:
    ...
    mutable Vec tmp_2;
    mutable unsigned int flag_tmp_2;
    ...
    void assemble_diagonal_cell_term_velocity_update(const MatrixFree<dim, Number>&               data,
                                                     Vec&                                         dst,
                                                     const Vec&                                   src,
                                                     const std::pair<unsigned int, unsigned int>& cell_range) const;
    void assemble_diagonal_face_term_velocity_update(const MatrixFree<dim, Number>&               data,
                                                     Vec&                                         dst,
                                                     const Vec&                                   src,
                                                     const std::pair<unsigned int, unsigned int>& face_range) const;
    void assemble_diagonal_boundary_term_velocity_update(const MatrixFree<dim, Number>&               data,
                                                         Vec&                                         dst,
                                                         const Vec&                                   src,
                                                         const std::pair<unsigned int, unsigned int>& face_range) const;
  };
  ...
  HYPERBOLICOperator(): MatrixFreeOperators::Base<dim, Vec>(), Ma(), Fr(), dt(), gamma(2.0 - std::sqrt(2.0)),
                        ...
                        a33_tilde(1.0 - std::sqrt(2)/2.0), HYPERBOLIC_stage(1), NS_stage(1), flag_tmp_2(0) {}
    ...
    if(HYPERBOLIC_stage == 1) {
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& rho_old_D        = phi_rho_old.get_value(q);
          const auto& u_old_D          = phi_u_old.get_value(q) - 2.0*scalar_product(phi_u_old.get_value(q), n_plus)*n_plus;

          const auto& avg_flux = 0.5*(phi_rho_old.get_value(q)*phi_u_old.get_value(q) + rho_old_D*u_old_D);

          phi.submit_value(-a21*dt*scalar_product(avg_flux, n_plus), q);
        }
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& rho_old_D        = phi_rho_old.get_value(q);
          const auto& u_old_D          = phi_u_old.get_value(q) - 2.0*scalar_product(phi_u_old.get_value(q), n_plus)*n_plus;
          const auto& rho_tmp_2_D      = phi_rho_tmp_2.get_value(q);
          const auto& u_tmp_2_D        = phi_u_tmp_2.get_value(q) - 2.0*scalar_product(phi_u_tmp_2.get_value(q), n_plus)*n_plus;
          ...
          phi.submit_value(-a31*dt*scalar_product(avg_flux_old, n_plus)
                           -a32*dt*scalar_product(avg_flux_tmp_2, n_plus), q);
        }
        phi.integrate_scatter(true, false, dst);
      }
    }
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          phi.submit_value(rho_old*E_old -
                           ...
                           a21*dt*Ma*Ma/(Fr*Fr)*rho_old*u_old[dim - 1], q);
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          phi.submit_value(rho_old*E_old -
                           ...
                           a31*dt*Ma*Ma/(Fr*Fr)*rho_old*u_old[dim - 1] -
                           a32*dt*Ma*Ma/(Fr*Fr)*rho_tmp_2*u_tmp_2[dim - 1], q);
    ...
    if(HYPERBOLIC_stage == 1) {
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& E_old            = 1.0/(EquationData::Cp_Cv - 1.0)*pres_old/rho_old
                                       + 0.5*Ma*Ma*scalar_product(u_old, u_old);
          const auto& E_old_D          = E_old;
          const auto& pres_old_D       = rho_old_D*(E_old_D - 0.5*Ma*Ma*scalar_product(u_old_D, u_old_D))*(EquationData::Cp_Cv - 1.0);
          ...
          phi.submit_value(-a21*dt*Ma*Ma*scalar_product(avg_kinetic_old, n_plus)
                           -a21_tilde*dt*scalar_product(avg_enthalpy_old, n_plus), q);
        }
        phi.integrate_scatter(true, false, dst);
      }
    }
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          phi.submit_value(-a31*dt*Ma*Ma*scalar_product(avg_kinetic_old, n_plus)
                           -a31_tilde*dt*scalar_product(avg_enthalpy_old, n_plus)
                           -a32*dt*Ma*Ma*scalar_product(avg_kinetic_tmp_2, n_plus)
                           -a32_tilde*dt*scalar_product(avg_enthalpy_tmp_2, n_plus), q);
        }
        phi.integrate_scatter(true, false, dst);
      }
    }
  ...
  // Assemble diagonal face term for the contribution due to internal energy
  //
  template<int dim, int fe_degree_rho, int fe_degree_T, int fe_degree_u,
           int n_q_points_1d_rho, int n_q_points_1d_T, int n_q_points_1d_u, typename Vec, typename Number>
  void HYPERBOLICOperator<dim, fe_degree_rho, fe_degree_T, fe_degree_u,
                          n_q_points_1d_rho, n_q_points_1d_T, n_q_points_1d_u, Vec, Number>::
  assemble_diagonal_face_term_internal_energy(const MatrixFree<dim, Number>&               data,
                                              Vec&                                         dst,
                                              const Vec&                                   src,
                                              const std::pair<unsigned int, unsigned int>& face_range) const {
    FEFaceEvaluation<dim, fe_degree_T, n_q_points_1d_T, 1, Number> phi_p(data, true, 1),
  ...
  assemble_cell_term_enthalpy(const MatrixFree<dim, Number>&               data,
    ...
    for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
      ...
      for(unsigned int q = 0; q < phi_src.n_q_points; ++q) {
  ...
  assemble_face_term_enthalpy(const MatrixFree<dim, Number>&               data,
    ...
    for(unsigned int face = face_range.first; face < face_range.second; ++face) {
      ...
      for(unsigned int q = 0; q < phi_src_p.n_q_points; ++q) {
        const auto& n_plus            = phi_p.get_normal_vector(q);
  ...
  assemble_boundary_term_enthalpy(const MatrixFree<dim, Number>&               data,
    ...
    for(unsigned int face = face_range.first; face < face_range.second; ++face) {
      ...
      for(unsigned int q = 0; q < phi_src.n_q_points; ++q) {
        ...
        const auto& E_fixed           = 1.0/(EquationData::Cp_Cv - 1.0)*pres_fixed/rho_for_fixed
                                      + 0.5*Ma*Ma*scalar_product(phi_src.get_value(q), phi_src.get_value(q));
        const auto& E_fixed_D         = E_fixed;
        const auto& u_D               = phi_src.get_value(q) - 2.0*scalar_product(phi_src.get_value(q), n_plus)*n_plus;
        const auto& pres_fixed_D      = (EquationData::Cp_Cv - 1.0)*rho_for_fixed*(E_fixed_D - 0.5*Ma*Ma*scalar_product(u_D, u_D));

        const auto& avg_flux_enthalpy = 0.5*EquationData::Cp_Cv/(EquationData::Cp_Cv - 1.0)*
                                        (pres_fixed*phi_src.get_value(q) + pres_fixed_D*u_D);
  ...
  assemble_boundary_term_pressure(const MatrixFree<dim, Number>&               data,
    ...
    for(unsigned int face = face_range.first; face < face_range.second; ++face) {
      ...
      for(unsigned int q = 0; q < phi_src.n_q_points; ++q) {
        ...
        const auto& rho_for_fixed   = phi_rho_for_fixed.get_value(q);
        const auto& u_fixed         = phi_u_fixed.get_value(q);
        const auto& E_fixed         = 1.0/(EquationData::Cp_Cv - 1.0)*phi_src.get_value(q)/rho_for_fixed
                                    + 0.5*Ma*Ma*scalar_product(u_fixed, u_fixed);
        const auto& rho_for_fixed_D = rho_for_fixed;
        const auto& u_fixed_D       = u_fixed - 2.0*scalar_product(u_fixed, n_plus)*n_plus;
        const auto& E_fixed_D       = E_fixed;
        const auto& pres_fixed_D    = (EquationData::Cp_Cv - 1.0)*
                                      rho_for_fixed_D*(E_fixed_D - 0.5*Ma*Ma*scalar_product(u_fixed_D, u_fixed_D));
    ...
    if(HYPERBOLIC_stage == 1) {
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          phi.submit_value(rho_old*u_old - a21*dt/(Fr*Fr)*rho_old*e_k, q);
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          phi.submit_value(rho_old*u_old - a31*dt/(Fr*Fr)*rho_old*e_k - a32*dt/(Fr*Fr)*rho_tmp_2*e_k, q);
    ...
    if(HYPERBOLIC_stage == 1) {
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& rho_tmp_2_D      = phi_rho_for_fixed.get_value(q);
          const auto& u_tmp_2_D        = phi.get_value(q) - 2.0*scalar_product(phi.get_value(q), n_plus)*n_plus;
          ...
          const auto& u_fixed_D     = u_fixed - 2.0*scalar_product(u_fixed, n_plus)*n_plus;
          const auto& pres_fixed_D  = (EquationData::Cp_Cv - 1.0)*
                                      rho_tmp_2_D*(E_fixed_D - 0.5*Ma*Ma*scalar_product(u_fixed_D, u_fixed_D));
          const auto& lambda_fixed  = std::max(scalar_product(u_fixed, u_fixed) +
                                               1.0/Ma*
                                               std::sqrt(EquationData::Cp_Cv*pres_fixed/rho_for_fixed),
                                               scalar_product(u_fixed_D, u_fixed_D) +
                                               1.0/Ma*
                                               std::sqrt(EquationData::Cp_Cv*pres_fixed_D/rho_tmp_2_D));

          phi.submit_value(a22_tilde*dt*0.5*lambda_fixed*(rho_for_fixed*phi.get_value(q) - rho_tmp_2_D*u_tmp_2_D), q);
        }
        phi.integrate_scatter(true, false, dst);
      }
    }
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          auto        rho_curr_D       = phi_rho_for_fixed.get_value(q);
          auto        u_curr_D         = phi.get_value(q) - 2.0*scalar_product(phi.get_value(q), n_plus)*n_plus;
          ...
          const auto& u_fixed_D     = u_fixed - 2.0*scalar_product(u_fixed, n_plus)*n_plus;
          const auto& pres_fixed_D  = (EquationData::Cp_Cv - 1.0)*
                                      rho_curr_D*(E_fixed_D - 0.5*Ma*Ma*scalar_product(u_fixed_D, u_fixed_D));
          const auto& lambda_fixed  = std::max(scalar_product(u_fixed, u_fixed) +
                                               1.0/Ma*
                                               std::sqrt(EquationData::Cp_Cv*pres_fixed/rho_for_fixed),
                                               scalar_product(u_fixed_D, u_fixed_D) +
                                               1.0/Ma*
                                               std::sqrt(EquationData::Cp_Cv*pres_fixed_D/rho_curr_D));

          phi.submit_value(a33_tilde*dt*0.5*lambda_fixed*(rho_for_fixed*phi.get_value(q) - rho_curr_D*u_curr_D), q);
        }
        phi.integrate_scatter(true, false, dst);
      }
    }
  ...
  void HYPERBOLICOperator<dim, fe_degree_rho, fe_degree_T, fe_degree_u,
                          n_q_points_1d_rho, n_q_points_1d_T, n_q_points_1d_u, Vec, Number>::
  assemble_diagonal_cell_term_velocity_update(const MatrixFree<dim, Number>&               data,
                                              Vec&                                         dst,
                                              const Vec&                                   src,
                                              const std::pair<unsigned int, unsigned int>& cell_range) const {
    FEEvaluation<dim, fe_degree_u, n_q_points_1d_u, dim, Number> phi(data, 0);
  ...
  // Assemble diagonal face term for the velocity update
  //
  template<int dim, int fe_degree_rho, int fe_degree_T, int fe_degree_u,
           int n_q_points_1d_rho, int n_q_points_1d_T, int n_q_points_1d_u, typename Vec, typename Number>
  void HYPERBOLICOperator<dim, fe_degree_rho, fe_degree_T, fe_degree_u,
                          n_q_points_1d_rho, n_q_points_1d_T, n_q_points_1d_u, Vec, Number>::
  assemble_diagonal_face_term_velocity_update(const MatrixFree<dim, Number>&               data,
                                              Vec&                                         dst,
                                              const Vec&                                   src,
                                              const std::pair<unsigned int, unsigned int>& face_range) const {
    FEFaceEvaluation<dim, fe_degree_u, n_q_points_1d_u, dim, Number> phi_p(data, true, 0),
  ...
  // Assemble diagonal boundary term for the velocity update
  //
  template<int dim, int fe_degree_rho, int fe_degree_T, int fe_degree_u,
           int n_q_points_1d_rho, int n_q_points_1d_T, int n_q_points_1d_u, typename Vec, typename Number>
  void HYPERBOLICOperator<dim, fe_degree_rho, fe_degree_T, fe_degree_u,
                          n_q_points_1d_rho, n_q_points_1d_T, n_q_points_1d_u, Vec, Number>::
  assemble_diagonal_boundary_term_velocity_update(const MatrixFree<dim, Number>&               data,
                                                  Vec&                                         dst,
                                                  const Vec&                                   src,
                                                  const std::pair<unsigned int, unsigned int>& face_range) const {
    FEFaceEvaluation<dim, fe_degree_u, n_q_points_1d_u, dim, Number> phi(data, true, 0),
    ...
    for(unsigned int face = face_range.first; face < face_range.second; ++face) {
      ...
      for(unsigned int i = 0; i < phi.dofs_per_component; ++i) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& rho_for_fixed_D  = phi_rho_for_fixed.get_value(q);
          const auto& u_D              = phi.get_value(q) - 2.0*scalar_product(phi.get_value(q), n_plus)*n_plus;
          ...
          const auto& lambda_fixed     = std::max(scalar_product(u_fixed, u_fixed) +
                                                  ...
                                                  scalar_product(u_fixed_D, u_fixed_D) +
                                                  1.0/Ma*
                                                  std::sqrt(EquationData::Cp_Cv*pres_fixed_D/rho_for_fixed_D));

          phi.submit_value(coeff*dt*0.5*lambda_fixed*(rho_for_fixed*phi.get_value(q) - rho_for_fixed_D*u_D), q);
        }
        phi.integrate(true, false);
        diagonal[i] = phi.get_dof_value(i);
      }
  ...
  void HYPERBOLICOperator<dim, fe_degree_rho, fe_degree_T, fe_degree_u,
                          n_q_points_1d_rho, n_q_points_1d_T, n_q_points_1d_u, Vec, Number>::
  compute_diagonal() {
    Assert(NS_stage == 2 || NS_stage == 3, ExcInternalError());
    if(NS_stage == 2) {
      ...
      Vec dummy_inverse_A;
      dummy_inverse_A.reinit(tmp_1.local_size());
      DiagonalMatrix<Vec> inverse_diagonal_A;
      auto& inverse_diagonal_A_entries = inverse_diagonal_A.get_vector();
      this->data->initialize_dof_vector(inverse_diagonal_A_entries, 0);
      this->data->loop(&HYPERBOLICOperator::assemble_diagonal_cell_term_velocity_update,
                       &HYPERBOLICOperator::assemble_diagonal_face_term_velocity_update,
                       &HYPERBOLICOperator::assemble_diagonal_boundary_term_velocity_update,
                       this, inverse_diagonal_A_entries, dummy_inverse_A, false,
                       MatrixFree<dim, Number>::DataAccessOnFaces::unspecified,
                       MatrixFree<dim, Number>::DataAccessOnFaces::unspecified);
      for(unsigned int i = 0; i < inverse_diagonal_A_entries.local_size(); ++i)
        inverse_diagonal_A_entries.local_element(i) = 1.0/inverse_diagonal_A_entries.local_element(i);

      Vec tmp_src;
      this->data->initialize_dof_vector(tmp_src, 1);
      Vec tmp_3;
      this->data->initialize_dof_vector(tmp_3, 1);
      Vec tmp_dst;
      this->data->initialize_dof_vector(tmp_dst, 1);

      if(Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 0)
        std::cout<<inverse_diagonal.local_size()<<std::endl;
      ...
      for(unsigned int i = 0; i < inverse_diagonal.local_size(); ++i) {
        ...
        tmp_src = 0;
        tmp_src.local_element(i) = 1.0;
        this->vmult_pressure(tmp_1, tmp_src);

        inverse_diagonal_A.vmult(tmp_2, tmp_1);

        this->vmult_enthalpy(tmp_3, tmp_2);

        tmp_dst.local_element(i) = tmp_3.local_element(i);
      }

      inverse_diagonal.add(1.0, tmp_dst);*/
  ...
  assemble_cell_term_velocity(const MatrixFree<dim, Number>&               data,
    ...
    for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
      ...
      for(unsigned int q = 0; q < phi.n_q_points; ++q) {
        ...
        auto div_u_times_identity        = outer_product(u, u);
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          phi.submit_gradient(-a21_tilde*dt*Ma*Ma/Re*(2.0*sym_grad_u_curr*u_curr - 2.0/3.0*div_u_curr*u_curr)
                              -a22_tilde*dt*Ma*Ma/Re*(2.0*sym_grad_u_tmp_2*u_tmp_2 - 2.0/3.0*div_u_tmp_2*u_tmp_2), q);
        }
        phi.integrate_scatter(true, true, dst);
      }
    }
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          phi.submit_gradient(-a31_tilde*dt*Ma*Ma/Re*(2.0*sym_grad_u_curr*u_curr - 2.0/3.0*div_u_curr*u_curr)
                              ...
                              -a33_tilde*dt*Ma*Ma/Re*(2.0*sym_grad_u_np1*u_np1 - 2.0/3.0*div_u_np1*u_np1), q);
        }
        phi.integrate_scatter(true, true, dst);
      }
    }
    ...
    if(VISCOUS_stage == 1) {
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi_p.n_q_points; ++q) {
          ...
          phi_p.submit_value(a21_tilde*dt*Ma*Ma/Re*scalar_product(avg_tau_u_curr, n_plus) +
                             a22_tilde*dt*Ma*Ma/Re*scalar_product(avg_tau_u_tmp_2, n_plus), q);
          phi_m.submit_value(-a21_tilde*dt*Ma*Ma/Re*scalar_product(avg_tau_u_curr, n_plus)
                             -a22_tilde*dt*Ma*Ma/Re*scalar_product(avg_tau_u_tmp_2, n_plus), q);
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi_p.n_q_points; ++q) {
          ...
          phi_p.submit_value(a31_tilde*dt*Ma*Ma/Re*scalar_product(avg_tau_u_curr, n_plus) +
                             a32_tilde*dt*Ma*Ma/Re*scalar_product(avg_tau_u_tmp_2, n_plus) +
                             a33_tilde*dt*Ma*Ma/Re*scalar_product(avg_tau_u_np1, n_plus), q);
          phi_m.submit_value(-a31_tilde*dt*Ma*Ma/Re*scalar_product(avg_tau_u_curr, n_plus) +
                             -a32_tilde*dt*Ma*Ma/Re*scalar_product(avg_tau_u_tmp_2, n_plus) +
                             -a33_tilde*dt*Ma*Ma/Re*scalar_product(avg_tau_u_np1, n_plus), q);
        }
        phi_p.integrate_scatter(true, false, dst);
    ...
    if(VISCOUS_stage == 1) {
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          phi.submit_value(a21_tilde*dt*Ma*Ma/Re*scalar_product(tau_u_curr, n_plus) +
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          phi.submit_value(a31_tilde*dt*Ma*Ma/Re*scalar_product(tau_u_curr, n_plus) +
  ...
  NavierStokesProjection<dim>::NavierStokesProjection(RunTimeParameters::Data_Storage& data):
    ...
    HYPERBOLIC_stage(1),
    VISCOUS_stage(1),             //--- Initialize the flag for the TR_BDF2 stage
    Ma(data.Mach),
    Fr(data.Froude),
    Re(data.Reynolds),
    dt(data.dt),
  ...
  void NavierStokesProjection<dim>::pressure_fixed_point() {
    ...
    SolverControl solver_control_schur(vel_max_its, 1e-12);
    ...
    PreconditionJacobi<HYPERBOLICOperator<dim,
                                          ...
                                          LinearAlgebra::distributed::Vector<double>, double>> preconditioner;
    euler_matrix.compute_diagonal();
    preconditioner.initialize(euler_matrix);
    gmres_schur.solve(euler_matrix, tmp_1, rhs_u, preconditioner);
    ...
    euler_matrix.compute_diagonal();
    preconditioner.initialize(euler_matrix);
    //const auto& lin_op = LinearOperator(euler_matrix);
    ...
    gmres.solve(euler_matrix, pres_fixed, rhs_pres, preconditioner);
  }
  ...
  void NavierStokesProjection<dim>::update_velocity() {
    ...
    gmres.solve(euler_matrix, u_fixed, rhs_u, preconditioner);
  }
  ...
  void NavierStokesProjection<dim>::diffusion_step() {
    ...
    if(VISCOUS_stage == 1)
      navier_stokes_matrix.vmult_rhs_velocity(rhs_u, {rho_curr, u_curr, pres_curr});
    else
      navier_stokes_matrix.vmult_rhs_velocity(rhs_u, {rho_curr, u_curr, pres_curr,
                                                      u_tmp_2, pres_tmp_2});
  ...
  //
  template<int dim>
  void NavierStokesProjection<dim>::pressure_step() {
    ...
    if(VISCOUS_stage == 1)
      navier_stokes_matrix.vmult_rhs_temperature(rhs_pres, {rho_curr, u_curr, pres_curr, u_tmp_2});
    else
      navier_stokes_matrix.vmult_rhs_temperature(rhs_pres, {rho_curr, u_curr, pres_curr,
  ...
  void NavierStokesProjection<dim>::run(const bool verbose, const unsigned int output_interval) {
    ...
    while(std::abs(T - time) > 1e-10) {
      ...
      pres_curr.equ(1.0, pres_fixed);
      u_curr.equ(1.0, u_fixed);
      VISCOUS_stage = 1; //--- Flag to pass at first stage of viscous part
      ...
      verbose_cout << "  Update velocity stage 1 viscous" << std::endl;
      diffusion_step();
      verbose_cout << "  Update pressure stage 1 viscous" << std::endl;
      pressure_step();
      VISCOUS_stage = 2;

      //---Second stage of VISCOUS operator
      navier_stokes_matrix.set_VISCOUS_stage(VISCOUS_stage);
      verbose_cout << "  Update velocity stage 2 viscous" << std::endl;
      diffusion_step();
      verbose_cout << "  Update pressure stage 2 viscous" << std::endl;
      pressure_step();
      HYPERBOLIC_stage = 1;
