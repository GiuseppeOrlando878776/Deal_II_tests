...
namespace Step35 {
  ...
  namespace EquationData {
    ...
    double Pressure<dim>::value(const Point<dim>&  p, const unsigned int component) const {
      ...
      return -std::sin(p(0))*std::cos(p(2)) -std::sin(p(1))*std::cos(p(0)) -std::sin(p(2))*std::cos(p(1));
    }
    ...
    if(TR_BDF2_stage == 1) {
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          auto u_ex                      = Tensor<1, dim, VectorizedArray<Number>>();
  ...
  void NavierStokesProjection<dim>::create_triangulation_and_dofs(const unsigned int n_refines) {
    ...
    dt = CFL*2.0*numbers::PI/(2.0*std::pow(2, n_refines)*(EquationData::degree_p + 1));
