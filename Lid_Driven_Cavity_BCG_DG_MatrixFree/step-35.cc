...
namespace Step35 {
  ...
  assemble_rhs_cell_term_velocity(const MatrixFree<dim, Number>&               data,
                                  ...
                                  const std::pair<unsigned int, unsigned int>& cell_range) const {
    if(TR_BDF2_stage == 1) {
      FEEvaluation<dim, fe_degree_v, n_q_points_1d_v, dim, Number> phi(data, 0), phi_old(data, 0);
  ...
  assemble_boundary_term_velocity(const MatrixFree<dim, Number>&               data,
                                  ...
                                  const std::pair<unsigned int, unsigned int>& face_range) const {
    if(TR_BDF2_stage == 1) {
      FEFaceEvaluation<dim, fe_degree_v, n_q_points_1d_v, dim, Number> phi(data, true, 0), phi_old(data, true, 0),
                                                                       phi_old_extr(data, true, 0);
