      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          if(boundary_id != 3) {
            ...
            phi.submit_value(-a22/Re*grad_u_int*n_plus + a22*tensor_product_u_int*n_plus, q);
            phi.submit_normal_derivative(0.0*u_int, q);
          }
        }
        phi.integrate_scatter(true, true, dst);
      }
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          if(boundary_id != 3) {
            ...
            phi.submit_value(-a33/Re*grad_u*n_plus + a33*tensor_product_u*n_plus, q);
            phi.submit_normal_derivative(0.0*u, q);
          }
        }
        phi.integrate_scatter(true, true, dst);
      }
