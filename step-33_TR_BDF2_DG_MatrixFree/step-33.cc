...
namespace Step33 {
  ...
  private:
    ...
    SystemMatrixType matrix_free_explicit;
  ...
  void ConservationLaw<dim>::setup_system() {
    ...
    std::shared_ptr<MatrixFree<dim, double>> matrix_free_explicit_storage(new MatrixFree<dim, double>());
  ...
  void ConservationLaw<dim>::assemble_explicit_system(const double current_time) {
    ...
    matrix_free_explicit.initialize_dof_vector(old_solution_tmp1);
    matrix_free_explicit.initialize_dof_vector(intermediate_solution_tmp1);
    ...
    matrix_free_explicit.vmult_explicit(current_time, {old_solution_tmp1, intermediate_solution_tmp1}, right_hand_side_explicit);
  }
  ...
  void ConservationLaw<dim>::assemble_cell_term(const FEValues<dim>& fe_v) {
    ...
    std::vector<Tensor<1, EulerEquations<dim>::n_components,
                          Tensor<1, dim, Sacado::Fad::DFad<double>>>> flux(n_q_points);
    std::vector<Tensor<1, EulerEquations<dim>::n_components, Sacado::Fad::DFad<double>>> forcing(n_q_points);
  ...
  void ConservationLaw<dim>::assemble_face_term(const unsigned int           face_no,
    ...
    std::vector<Tensor<1, EulerEquations<dim>::n_components, Sacado::Fad::DFad<double>>> normal_fluxes(n_q_points);

    for(unsigned int q = 0; q < n_q_points; ++q)
      EulerEquations<dim>::numerical_normal_flux(fe_v.normal_vector(q), Wplus[q], Wminus[q],
                                                 matrix_free_explicit.get_lambda_old(), normal_fluxes[q]);
  ...
  void ConservationLaw<dim>::run() {
    ...
    while(time < parameters.final_time) {
      ...
      //--- TR_BDF2 Newton loops
        ...
        matrix_free_explicit.set_TR_BDF2_stage(TR_BDF2_stage);
        assemble_explicit_system(time);
