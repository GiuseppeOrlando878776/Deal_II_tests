...
namespace Step33
{
  ...
  namespace Parameters
  {
    ...
    void AllParameters<dim>::parse_parameters(ParameterHandler &prm)
    {
      ...
      prm.enter_subsection("time stepping");
      {
        ...
        time_integration_scheme = prm.get("time integration scheme"); //--- Read the time integration scheme chosen and check the correctness
  ...
  private:
    ...
    Vector<double> intermediate_solution; //--- Extra variable to store intermediate solution of TR_BDF2
    ...
    const std::vector<types::global_dof_index> &dof_indices)
  {
    ...
    Table<2, Sacado::Fad::DFad<double>> W_int(n_q_points, EulerEquations<dim>::n_components);
    ...
    for (unsigned int i = 0; i < dofs_per_cell; ++i)
      //--- Adjusting for TR_BDF2
      if(parameters.time_integration_scheme == "Theta_Method" || TR_BDF2_stage == 2)
        independent_local_dof_values[i] = current_solution(dof_indices[i]);
      else
        independent_local_dof_values[i] = intermediate_solution(dof_indices[i]);
      ...
      for (unsigned int i = 0; i < dofs_per_cell; i++)
        {
          ...
          if(parameters.time_integration_scheme == "Theta_Method" || TR_BDF2_stage == 2)
            ...
            independent_neighbor_dof_values[i] = intermediate_solution(dof_indices_neighbor[i]);
  ...
  ConservationLaw<dim>::refine_grid(const Vector<double> &refinement_indicators)
  {
    ...
    //--- Adjusting for TR_BDF2
    intermediate_solution.reinit(dof_handler.n_dofs());
    intermediate_solution = old_solution;
  ...
  void ConservationLaw<dim>::run()
  {
    ...
    intermediate_solution.reinit(dof_handler.n_dofs()); //--- Adjusting for TR_BDF2
    current_solution.reinit(dof_handler.n_dofs());
    ...
    VectorTools::interpolate(dof_handler,
                             ...
                             old_solution);
    current_solution = old_solution;
    intermediate_solution = old_solution; //--- Adjusting for TR_BDF2
        ...
        //--- TR_BDF2 Newton loops
          ...
          while(true) {
            ...
            if(std::fabs(res_norm) < 1e-10) {
                ...
                intermediate_solution += newton_update;
          ...
          nonlin_iter = 0; //--- Reset number of Newton iterations
          TR_BDF2_stage = 2; //--- Pass to second stage
          current_solution = intermediate_solution;
          //--- Second stage TR_BDF2 Newton loop
          while(true) {
            system_matrix = 0;
          ...
          TR_BDF2_stage = 1; //--- Prepare to pass to frsit stage at next time-step
        }
