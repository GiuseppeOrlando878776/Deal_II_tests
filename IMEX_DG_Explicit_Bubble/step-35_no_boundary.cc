    ...
    if(VISCOUS_stage == 1) {
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          auto div_u_tmp_2_times_identity        = outer_product(u_curr, u_curr);
          div_u_tmp_2_times_identity = 0;
          for(unsigned int d = 0; d < dim; ++d)
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi_p.n_q_points; ++q) {
          ...
          phi_p.submit_value(a31_tilde*dt/Re*(0.5*(2.0*sym_grad_u_curr_p + 2.0*sym_grad_u_curr_m)*n_plus -
                             ...
                             a32_tilde*dt/Re*(0.5*2.0*(sym_grad_u_tmp_2_p + sym_grad_u_tmp_2_m)*n_plus -
