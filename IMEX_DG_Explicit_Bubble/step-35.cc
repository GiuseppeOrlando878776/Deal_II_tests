...
namespace Step35 {
  ...
  assemble_rhs_face_term_rho_projection(const MatrixFree<dim, Number>&               data,
    ...
    if(HYPERBOLIC_stage == 1) {
      ...
      FEFaceEvaluation<dim, fe_degree_T, n_q_points_1d_T, 1, Number> phi_E_old_p(data, true, 1),
                                                                     phi_E_old_m(data, false, 1);
        ...
        for(unsigned int q = 0; q < phi_p.n_q_points; ++q) {
          ...
          const auto& pres_old_p = (EquationData::Cp_Cv - 1.0)*
                                   (phi_E_old_p.get_value(q) -
                                    0.5*Ma*Ma/phi_rho_old_p.get_value(q)*
          ...
          const auto& pres_old_m = (EquationData::Cp_Cv - 1.0)*
                                   (phi_E_old_m.get_value(q) -
                                    0.5*Ma*Ma/phi_rho_old_m.get_value(q)*
      ...
      FEFaceEvaluation<dim, fe_degree_T, n_q_points_1d_T, 1, Number> phi_E_old_p(data, true, 1),
                                                                     phi_E_old_m(data, false, 1),
                                                                     phi_E_tmp_2_p(data, true, 1),
                                                                     phi_E_tmp_2_m(data, false, 1);
        ...
        for(unsigned int q = 0; q < phi_p.n_q_points; ++q) {
          ...
          const auto& pres_tmp_2_p   = (EquationData::Cp_Cv - 1.0)*
                                       (phi_E_tmp_2_p.get_value(q) -
                                        0.5*Ma*Ma/phi_rho_tmp_2_p.get_value(q)*
          ...
          const auto& pres_tmp_2_m   = (EquationData::Cp_Cv - 1.0)*
                                       (phi_E_tmp_2_m.get_value(q) -
                                        0.5*Ma*Ma/phi_rho_tmp_2_m.get_value(q)*
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& pres_old     = (EquationData::Cp_Cv - 1.0)*
                                     (phi_E_old.get_value(q) -
                                      0.5*Ma*Ma/phi_rho_old.get_value(q)*
                                      scalar_product(phi_u_old.get_value(q), phi_u_old.get_value(q)));
          const auto& pres_old_D   = (EquationData::Cp_Cv - 1.0)*
                                     (phi_E_old.get_value(q) - 0.5*Ma*Ma/rho_old_D*scalar_product(u_old_D, u_old_D));
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& pres_tmp_2     = (EquationData::Cp_Cv - 1.0)*
                                       (phi_E_tmp_2.get_value(q) -
                                        0.5*Ma*Ma/phi_rho_tmp_2.get_value(q)*
                                        scalar_product(phi_u_tmp_2.get_value(q), phi_u_tmp_2.get_value(q)));
          const auto& pres_tmp_2_D   = (EquationData::Cp_Cv - 1.0)*
                                       (phi_E_tmp_2.get_value(q) - 0.5*Ma*Ma/rho_tmp_2_D*scalar_product(u_tmp_2_D, u_tmp_2_D));
    ...
    if(HYPERBOLIC_stage == 1) {
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& pres_old = (EquationData::Cp_Cv - 1.0)*(E_old - 0.5*Ma*Ma/rho_old*scalar_product(u_old, u_old));
          ...
          phi.submit_gradient(a21*dt*(Ma*Ma*0.5*scalar_product(u_old/rho_old, u_old/rho_old)*u_old +
                                      (E_old - 0.5*Ma*Ma*scalar_product(u_old, u_old)/rho_old + pres_old)*u_old/rho_old), q);
        }
        phi.integrate_scatter(true, true, dst);
      }
    }
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& pres_tmp_2 = (EquationData::Cp_Cv - 1.0)*(E_tmp_2 - 0.5*Ma*Ma/rho_tmp_2*scalar_product(u_tmp_2, u_tmp_2));
          ...
          phi.submit_gradient(a31*dt*(Ma*Ma*0.5*scalar_product(u_old/rho_old, u_old/rho_old)*u_old +
                                      (E_old - 0.5*Ma*Ma*scalar_product(u_old, u_old)/rho_old + pres_old)*u_old/rho_old) +
                              a32*dt*(Ma*Ma*0.5*scalar_product(u_tmp_2/rho_tmp_2, u_tmp_2/rho_tmp_2)*u_tmp_2 +
                                      (E_tmp_2 -
                                       0.5*Ma*Ma*scalar_product(u_tmp_2, u_tmp_2)/rho_tmp_2 + pres_tmp_2)*u_tmp_2/rho_tmp_2), q);
        }
        phi.integrate_scatter(true, true, dst);
      }
    }
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& avg_enthalpy_old = 0.5*((phi_E_old.get_value(q) -
                                               ...
                                               phi_u_old.get_value(q)/phi_rho_old.get_value(q)  +
                                               (E_old_D - 0.5*Ma*Ma/rho_old_D*scalar_product(u_old_D, u_old_D) + pres_old_D)*
                                               u_old_D/rho_old_D);
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& avg_enthalpy_tmp_2 = 0.5*((phi_E_tmp_2.get_value(q) -
                                                 ...
                                                 phi_u_tmp_2.get_value(q)/phi_rho_tmp_2.get_value(q)  +
                                                 (E_tmp_2_D - 0.5*Ma*Ma/rho_tmp_2_D*scalar_product(u_tmp_2_D, u_tmp_2_D) + pres_tmp_2_D)*
  ...
  assemble_rhs_cell_term_velocity_projection(const MatrixFree<dim, Number>&               data,
    ...
    for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
      ...
      phi_u_curr.gather_evaluate(src[1], true, false);
      phi.reinit(cell);
    ...
    for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
      ...
      for(unsigned int q = 0; q < phi.n_q_points; ++q)
        phi.submit_value((EquationData::Cp_Cv - 1.0)*
                         (phi_E.get_value(q) -
                          0.5*Ma*Ma/phi_rho.get_value(q)*scalar_product(phi_u.get_value(q), phi_u.get_value(q))), q);
  ...
  // @sect3{ <code>VISCOUSOperator::VISCOUSOperator</code> }
  //
  template<int dim, int fe_degree_rho, int fe_degree_T, int fe_degree_u,
           int n_q_points_1d_rho, int n_q_points_1d_T, int n_q_points_1d_u, typename Vec, typename Number>
  class VISCOUSOperator: public MatrixFreeOperators::Base<dim, Vec> {
  public:
    VISCOUSOperator();

    VISCOUSOperator(RunTimeParameters::Data_Storage& data);

    void set_dt(const double time_step);

    void set_Reynolds(const double Re_);

    void set_Prandtl(const double Pr_);

    void set_Mach(const double Ma_);

    void set_VISCOUS_stage(const unsigned int stage);

    void set_NS_stage(const unsigned int stage);

    void set_rho_curr(const Vec& src);
    ...
    void vmult_rhs_momentum_projection(Vec& dst, const std::vector<Vec>& src) const;

    void vmult_rhs_energy_projection(Vec& dst, const std::vector<Vec>& src) const;
  ...
  protected:
    ...
    unsigned int VISCOUS_stage;
    unsigned int NS_stage;

    virtual void apply_add(Vec& dst, const Vec& src) const override;

  private:
    const double theta_u = 0.0;
    const double theta_T = 1.0;
    ...
    void assemble_rhs_cell_term_momentum_projection(const MatrixFree<dim, Number>&               data,
                                                    Vec&                                         dst,
                                                    const std::vector<Vec>&                      src,
                                                    const std::pair<unsigned int, unsigned int>& cell_range) const;

    void assemble_cell_term_momentum_projection(const MatrixFree<dim, Number>&               data,
                                                Vec&                                         dst,
                                                const Vec&                                   src,
                                                const std::pair<unsigned int, unsigned int>& cell_range) const;

    void assemble_rhs_cell_term_energy_projection(const MatrixFree<dim, Number>&               data,
                                                  Vec&                                         dst,
                                                  const std::vector<Vec>&                      src,
                                                  const std::pair<unsigned int, unsigned int>& cell_range) const;

    void assemble_cell_term_energy_projection(const MatrixFree<dim, Number>&               data,
                                              Vec&                                         dst,
                                              const Vec&                                   src,
                                              const std::pair<unsigned int, unsigned int>& cell_range) const;
  };
  ...
  template<int dim, int fe_degree_rho, int fe_degree_T, int fe_degree_u,
           int n_q_points_1d_rho, int n_q_points_1d_T, int n_q_points_1d_u, typename Vec, typename Number>
  VISCOUSOperator<dim, fe_degree_rho, fe_degree_T, fe_degree_u,
                  n_q_points_1d_rho, n_q_points_1d_T, n_q_points_1d_u, Vec, Number>::
  VISCOUSOperator(): MatrixFreeOperators::Base<dim, Vec>(), Re(), Pr(), Ma(), dt(),
                     gamma(2.0 - std::sqrt(2.0)), a21_tilde(0.5*gamma), a22_tilde(0.5*gamma),
  ...
  VISCOUSOperator<dim, fe_degree_rho, fe_degree_T, fe_degree_u,
                  n_q_points_1d_rho, n_q_points_1d_T, n_q_points_1d_u, Vec, Number>::
  VISCOUSOperator(RunTimeParameters::Data_Storage& data): MatrixFreeOperators::Base<dim, Vec>(),
                                                          Re(data.Reynolds), Pr(data.Prandtl), Ma(data.Mach), dt(data.dt),
                                                          gamma(2.0 - std::sqrt(2.0)), a21_tilde(0.5*gamma),
                                                          a22_tilde(0.5*gamma), a31_tilde(std::sqrt(2)/4.0),
                                                          a32_tilde(std::sqrt(2)/4.0), a33_tilde(1.0 - std::sqrt(2)/2.0),
                                                          VISCOUS_stage(1), NS_stage(1) {}
  ...
  template<int dim, int fe_degree_rho, int fe_degree_T, int fe_degree_u,
           int n_q_points_1d_rho, int n_q_points_1d_T, int n_q_points_1d_u, typename Vec, typename Number>
  void VISCOUSOperator<dim, fe_degree_rho, fe_degree_T, fe_degree_u,
                       n_q_points_1d_rho, n_q_points_1d_T, n_q_points_1d_u, Vec, Number>::
  set_dt(const double time_step) {
    dt = time_step;
  }
  ...
  void VISCOUSOperator<dim, fe_degree_rho, fe_degree_T, fe_degree_u,
                       n_q_points_1d_rho, n_q_points_1d_T, n_q_points_1d_u, Vec, Number>::
  set_Reynolds(const double Re_) {
    Re = Re_;
  }


  // Setter of Prandtl number
  //
  template<int dim, int fe_degree_rho, int fe_degree_T, int fe_degree_u,
           int n_q_points_1d_rho, int n_q_points_1d_T, int n_q_points_1d_u, typename Vec, typename Number>
  void VISCOUSOperator<dim, fe_degree_rho, fe_degree_T, fe_degree_u,
                       n_q_points_1d_rho, n_q_points_1d_T, n_q_points_1d_u, Vec, Number>::
  set_Prandtl(const double Pr_) {
    Pr = Pr_;
  }
  ...
  // Setter of current density for IMEX part
  //
  template<int dim, int fe_degree_rho, int fe_degree_T, int fe_degree_u,
           int n_q_points_1d_rho, int n_q_points_1d_T, int n_q_points_1d_u, typename Vec, typename Number>
  void VISCOUSOperator<dim, fe_degree_rho, fe_degree_T, fe_degree_u,
                       n_q_points_1d_rho, n_q_points_1d_T, n_q_points_1d_u, Vec, Number>::
  set_rho_curr(const Vec& src) {
    rho_curr = src;
    rho_curr.update_ghost_values();
  }
    ...
    if(VISCOUS_stage == 1) {
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& sym_grad_u_curr           = phi_u_curr.get_symmetric_gradient(q);
          const auto& div_u_curr                = phi_u_curr.get_divergence(q);
          auto div_u_curr_times_identity        = outer_product(u_curr, u_curr);
          div_u_curr_times_identity = 0;
          for(unsigned int d = 0; d < dim; ++d)
            div_u_curr_times_identity[d][d] = div_u_curr;

          phi.submit_value(rho_curr*u_curr, q);
          phi.submit_gradient(-a21_tilde*dt/Re*(2.0*sym_grad_u_curr - 2.0/3.0*div_u_curr_times_identity), q);
        }
        phi.integrate_scatter(true, true, dst);
      }
    }
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        phi_u_curr.gather_evaluate(src[1], true, true);
        phi_u_tmp_2.reinit(cell);
        phi_u_tmp_2.gather_evaluate(src[3], false, true);
        phi.reinit(cell);
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& sym_grad_u_tmp_2           = phi_u_tmp_2.get_symmetric_gradient(q);
          const auto& div_u_tmp_2                = phi_u_tmp_2.get_divergence(q);
          auto div_u_tmp_2_times_identity        = div_u_curr_times_identity;
          for(unsigned int d = 0; d < dim; ++d)
            div_u_tmp_2_times_identity[d][d] = div_u_tmp_2;

          phi.submit_value(rho_curr*u_curr, q);
          phi.submit_gradient(-a31_tilde*dt/Re*(2.0*sym_grad_u_curr - 2.0/3.0*div_u_curr_times_identity)
                              -a32_tilde*dt/Re*(2.0*sym_grad_u_tmp_2 - 2.0/3.0*div_u_tmp_2_times_identity), q);
        }
        phi.integrate_scatter(true, true, dst);
      }
    }
  ...
  assemble_rhs_face_term_velocity(const MatrixFree<dim, Number>&               data,
    ...
    if(VISCOUS_stage == 1) {
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        phi_u_curr_p.gather_evaluate(src[1], false, true);
        phi_u_curr_m.reinit(face);
        phi_u_curr_m.gather_evaluate(src[1], false, true);
        phi_p.reinit(face);
        phi_m.reinit(face);
        ...
        for(unsigned int q = 0; q < phi_p.n_q_points; ++q) {
          ...
          const auto& sym_grad_u_curr_p = phi_u_curr_p.get_symmetric_gradient(q);
          const auto& sym_grad_u_curr_m = phi_u_curr_m.get_symmetric_gradient(q);
          const auto& div_u_curr_p      = phi_u_curr_p.get_divergence(q);
          const auto& div_u_curr_m      = phi_u_curr_m.get_divergence(q);

          phi_p.submit_value(a21_tilde*dt/Re*(0.5*(2.0*sym_grad_u_curr_p + 2.0*sym_grad_u_curr_m)*n_plus -
                                              0.5*(2.0/3.0*div_u_curr_p + 2.0/3.0*div_u_curr_m)*n_plus), q);
          phi_m.submit_value(-a21_tilde*dt/Re*(0.5*(2.0*sym_grad_u_curr_p + 2.0*sym_grad_u_curr_m)*n_plus -
                                               0.5*(2.0/3.0*div_u_curr_p + 2.0/3.0*div_u_curr_m)*n_plus), q);
        }
        phi_p.integrate_scatter(true, false, dst);
        phi_m.integrate_scatter(true, false, dst);
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        phi_u_curr_m.gather_evaluate(src[1], false, true);
        phi_u_tmp_2_p.reinit(face);
        phi_u_tmp_2_p.gather_evaluate(src[3], false, true);
        phi_u_tmp_2_m.reinit(face);
        phi_u_tmp_2_m.gather_evaluate(src[3], false, true);
        phi_p.reinit(face);
        phi_m.reinit(face);
        ...
        for(unsigned int q = 0; q < phi_p.n_q_points; ++q) {
          ...
          const auto& sym_grad_u_tmp_2_p = phi_u_tmp_2_p.get_symmetric_gradient(q);
          const auto& sym_grad_u_tmp_2_m = phi_u_tmp_2_m.get_symmetric_gradient(q);
          const auto& div_u_tmp_2_p      = phi_u_tmp_2_p.get_divergence(q);
          const auto& div_u_tmp_2_m      = phi_u_tmp_2_m.get_divergence(q);

          phi_p.submit_value(a31_tilde*dt/Re*(0.5*(2.0*sym_grad_u_curr_p + 2.0*sym_grad_u_curr_m)*n_plus -
                                              0.5*(2.0/3.0*div_u_curr_p + 2.0/3.0*div_u_curr_m)*n_plus) +
                             a32_tilde*dt/Re*(0.5*(2.0*sym_grad_u_tmp_2_p + 2.0*sym_grad_u_tmp_2_m)*n_plus -
                                              0.5*(2.0/3.0*div_u_tmp_2_p + 2.0/3.0*div_u_tmp_2_m)*n_plus), q);
          phi_m.submit_value(-a31_tilde*dt/Re*(0.5*(2.0*sym_grad_u_curr_p + 2.0*sym_grad_u_curr_m)*n_plus -
                                               0.5*(2.0/3.0*div_u_curr_p + 2.0/3.0*div_u_curr_m)*n_plus)
                             -a32_tilde*dt/Re*(0.5*(2.0*sym_grad_u_tmp_2_p + 2.0*sym_grad_u_tmp_2_m)*n_plus -
                                               0.5*(2.0/3.0*div_u_tmp_2_p + 2.0/3.0*div_u_tmp_2_m)*n_plus), q);
        }
  ...
  assemble_rhs_boundary_term_velocity(const MatrixFree<dim, Number>&               data,
    ...
    if(VISCOUS_stage == 1) {
      FEFaceEvaluation<dim, fe_degree_u, n_q_points_1d_u, dim, Number> phi(data, true, 0),
                                                                       phi_u_curr(data, true, 0);

      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        phi_u_curr.reinit(face);
        phi_u_curr.gather_evaluate(src[1], false, true);
        phi.reinit(face);
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          phi.submit_value(a21_tilde*dt/Re*(2.0*sym_grad_u_curr*n_plus - 2.0/3.0*div_u_curr*n_plus), q);
        }
        phi.integrate_scatter(true, false, dst);
      }
    }
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          phi.submit_value(a31_tilde*dt/Re*(2.0*sym_grad_u_curr*n_plus - 2.0/3.0*div_u_curr*n_plus) +
                           a32_tilde*dt/Re*(2.0*sym_grad_u_tmp_2*n_plus - 2.0/3.0*div_u_tmp_2*n_plus), q);
        }
        phi.integrate_scatter(true, false, dst);
      }
    }
  ...
  vmult_rhs_velocity(Vec& dst, const std::vector<Vec>& src) const {
    for(unsigned int d = 0; d < src.size(); ++d)
      src[d].update_ghost_values();
    this->data->loop(&VISCOUSOperator::assemble_rhs_cell_term_velocity,
                     &VISCOUSOperator::assemble_rhs_face_term_velocity,
                     &VISCOUSOperator::assemble_rhs_boundary_term_velocity,
                     this, dst, src, true,
                     MatrixFree<dim, Number>::DataAccessOnFaces::unspecified,
                     MatrixFree<dim, Number>::DataAccessOnFaces::unspecified);
  }
  ...
  assemble_cell_term_velocity(const MatrixFree<dim, Number>&               data,
    ...
    const double coeff = (VISCOUS_stage == 1) ? a22_tilde*dt/Re : a33_tilde*dt/Re;

    for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
      phi_rho_curr.reinit(cell);
      phi_rho_curr.gather_evaluate(rho_curr, true, false);
      phi.reinit(cell);
      ...
      for(unsigned int q = 0; q < phi.n_q_points; ++q) {
        ...
        const auto& sym_grad_u           = phi.get_symmetric_gradient(q);
        const auto& div_u                = phi.get_divergence(q);
        auto div_u_times_identity        = outer_product(phi.get_value(q), phi.get_value(q));
        div_u_times_identity = 0;
        for(unsigned int d = 0; d < dim; ++d)
          div_u_times_identity[d][d] = div_u;

        phi.submit_value(rho_curr*u, q);
        phi.submit_gradient(coeff*(2.0*sym_grad_u - 2.0/3.0*div_u_times_identity), q);
      }
      phi.integrate_scatter(true, true, dst);
    }
  }
  ...
  assemble_face_term_velocity(const MatrixFree<dim, Number>&               data,
    ...
    for(unsigned int face = face_range.first; face < face_range.second; ++face) {
      ...
      for(unsigned int q = 0; q < phi_p.n_q_points; ++q) {
        ...
        const auto& sym_grad_u_p = phi_p.get_symmetric_gradient(q);
        const auto& sym_grad_u_m = phi_m.get_symmetric_gradient(q);
        const auto& div_u_p      = phi_p.get_divergence(q);
        const auto& div_u_m      = phi_m.get_divergence(q);
        auto avg_tau             = outer_product(phi_p.get_value(q), phi_p.get_value(q));
        avg_tau = 0;
        for(unsigned int d = 0; d < dim; ++d)
          avg_tau[d][d] = 0.5*(div_u_p + div_u_m);
        avg_tau = 0.5*(2.0*sym_grad_u_p + 2.0*sym_grad_u_m) - 2.0/3.0*avg_tau;

        const auto& jump_u = phi_p.get_value(q) - phi_m.get_value(q);

        auto u_dot_n_times_identity = avg_tau;
        u_dot_n_times_identity = 0;
        for(unsigned int d = 0; d < dim; ++d)
          u_dot_n_times_identity[d][d] = scalar_product(jump_u, n_plus);

        phi_p.submit_value(coeff*(-avg_tau*n_plus + coef_jump*jump_u), q);
        phi_m.submit_value(-coeff*(-avg_tau*n_plus + coef_jump*jump_u), q);
        phi_p.submit_gradient(-theta_u*coeff*
                               0.5*(outer_product(jump_u,n_plus) + outer_product(n_plus, jump_u) - 2.0/3.0*u_dot_n_times_identity), q);
        phi_m.submit_gradient(-theta_u*coeff*
                               0.5*(outer_product(jump_u,n_plus) + outer_product(n_plus, jump_u) - 2.0/3.0*u_dot_n_times_identity), q);
  ...
  assemble_boundary_term_velocity(const MatrixFree<dim, Number>&               data,
    ...
    for(unsigned int face = face_range.first; face < face_range.second; ++face) {
      ...
      for(unsigned int q = 0; q < phi.n_q_points; ++q) {
        ...
        const auto& u_D          = u - 2.0*scalar_product(u, n_plus)*n_plus;
        const auto& sym_grad_u_D = sym_grad_u -
                                   outer_product(n_plus, transpose(phi.get_gradient(q))*n_plus) -
                                   outer_product(transpose(phi.get_gradient(q))*n_plus, n_plus);
        const auto& div_u_D      = div_u - 2.0*scalar_product(n_plus, transpose(phi.get_gradient(q))*n_plus);
        auto avg_tau             = outer_product(u, u);
        avg_tau = 0;
        for(unsigned int d = 0; d < dim; ++d)
          avg_tau[d][d] = 0.5*(div_u + div_u_D);
        avg_tau = 0.5*(2.0*sym_grad_u + 2.0*sym_grad_u_D) - 2.0/3.0*avg_tau;

        const auto& jump_u = u - u_D;
        ...
        phi.submit_value(coeff*(-avg_tau*n_plus + coef_jump*jump_u), q);
        phi.submit_gradient(-theta_u*coeff*0.5*(outer_product(jump_u, n_plus) +
                                                outer_product(n_plus, jump_u) -
                                                2.0/3.0*u_dot_n_times_identity), q);
      }
      phi.integrate_scatter(true, true, dst);
    }
  }
  ...
  // Assemble rhs cell term for the temperature
  //
  template<int dim, int fe_degree_rho, int fe_degree_T, int fe_degree_u,
           int n_q_points_1d_rho, int n_q_points_1d_T, int n_q_points_1d_u, typename Vec, typename Number>
  void VISCOUSOperator<dim, fe_degree_rho, fe_degree_T, fe_degree_u,
  ...
  assemble_rhs_cell_term_temperature(const MatrixFree<dim, Number>&               data,
    ...
    if(VISCOUS_stage == 1) {
      FEEvaluation<dim, fe_degree_T, n_q_points_1d_T, 1, Number> phi(data, 1),
                                                                 phi_pres_curr(data, 1);
      FEEvaluation<dim, fe_degree_u, n_q_points_1d_u, dim, Number> phi_u_curr(data, 0),
                                                                   phi_u_tmp_2(data, 0);
      FEEvaluation<dim, fe_degree_rho, n_q_points_1d_rho, 1, Number> phi_rho_curr(data, 2);

      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        phi_rho_curr.reinit(cell);
        phi_rho_curr.gather_evaluate(src[0], true, true);
        phi_u_curr.reinit(cell);
        phi_u_curr.gather_evaluate(src[1], true, true);
        phi_pres_curr.reinit(cell);
        phi_pres_curr.gather_evaluate(src[2], true, true);
        phi_u_tmp_2.reinit(cell);
        phi_u_tmp_2.gather_evaluate(src[3], true, true);
        phi.reinit(cell);

        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          const auto& rho_curr         = phi_rho_curr.get_value(q);
          const auto& pres_curr        = phi_pres_curr.get_value(q);
          const auto& u_curr           = phi_u_curr.get_value(q);
          const auto& E_curr           = 1.0/(EquationData::Cp_Cv - 1.0)*pres_curr/rho_curr
                                       + 0.5*Ma*Ma*scalar_product(u_curr, u_curr);

          const auto& grad_rho_curr    = phi_rho_curr.get_gradient(q);
          const auto& grad_pres_curr   = phi_pres_curr.get_gradient(q);
          ...
          phi.submit_value(rho_curr*(E_curr - 0.5*Ma*Ma*scalar_product(u_tmp_2, u_tmp_2)), q);
          phi.submit_gradient(-a21_tilde*dt*Ma*Ma/Re*(2.0*sym_grad_u_curr*u_curr - 2.0/3.0*div_u_curr*u_curr)
                              -a22_tilde*dt*Ma*Ma/Re*(2.0*sym_grad_u_tmp_2*u_tmp_2 - 2.0/3.0*div_u_tmp_2*u_tmp_2)
                              -a21_tilde*dt/(Pr*Re*287.05)*(grad_pres_curr/rho_curr - grad_rho_curr*pres_curr/(rho_curr*rho_curr)), q);
        }
        phi.integrate_scatter(true, true, dst);
      }
    }
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        phi_pres_tmp_2.gather_evaluate(src[4], true, true);
        phi_u_np1.reinit(cell);
        phi_u_np1.gather_evaluate(src[5], true, true);
        phi.reinit(cell);
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& u_np1            = phi_u_np1.get_value(q);
          const auto& sym_grad_u_np1   = phi_u_np1.get_symmetric_gradient(q);
          const auto& div_u_np1        = phi_u_np1.get_divergence(q);

          const auto& grad_rho_curr    = phi_rho_curr.get_gradient(q);
          const auto& grad_pres_curr   = phi_pres_curr.get_gradient(q);

          const auto& pres_tmp_2        = phi_pres_tmp_2.get_value(q);
          const auto& grad_pres_tmp_2   = phi_pres_tmp_2.get_gradient(q);

          phi.submit_value(rho_curr*(E_curr - 0.5*scalar_product(u_np1, u_np1)), q);
          phi.submit_gradient(-a31_tilde*dt*Ma*Ma/Re*(2.0*sym_grad_u_curr*u_curr - 2.0/3.0*div_u_curr*u_curr)
                              -a32_tilde*dt*Ma*Ma/Re*(2.0*sym_grad_u_tmp_2*u_tmp_2 - 2.0/3.0*div_u_tmp_2*u_tmp_2)
                              -a33_tilde*dt*Ma*Ma/Re*(2.0*sym_grad_u_np1*u_np1 - 2.0/3.0*div_u_np1*u_np1)
                              -a31_tilde*dt/(Pr*Re*287.05)*(grad_pres_curr/rho_curr - grad_rho_curr*pres_curr/(rho_curr*rho_curr))
                              -a32_tilde*dt/(Pr*Re*287.05)*(grad_pres_tmp_2/rho_curr - grad_rho_curr*pres_tmp_2/(rho_curr*rho_curr)), q);
        }
        phi.integrate_scatter(true, true, dst);
      }
    }
  ...
  // Assemble rhs face term for the temperature
  //
  template<int dim, int fe_degree_rho, int fe_degree_T, int fe_degree_u,
           int n_q_points_1d_rho, int n_q_points_1d_T, int n_q_points_1d_u, typename Vec, typename Number>
  void VISCOUSOperator<dim, fe_degree_rho, fe_degree_T, fe_degree_u,
  ...
  assemble_rhs_face_term_temperature(const MatrixFree<dim, Number>&               data,
    ...
    if(VISCOUS_stage == 1) {
      FEFaceEvaluation<dim, fe_degree_T, n_q_points_1d_T, 1, Number> phi_p(data, true, 1),
                                                                     phi_m(data, false, 1),
                                                                     phi_pres_curr_p(data, true, 1),
                                                                     phi_pres_curr_m(data, false, 1);
      FEFaceEvaluation<dim, fe_degree_u, n_q_points_1d_u, dim, Number> phi_u_curr_p(data, true, 0),
                                                                       phi_u_curr_m(data, false, 0),
                                                                       phi_u_tmp_2_p(data, true, 0),
                                                                       phi_u_tmp_2_m(data, false, 0);
      FEFaceEvaluation<dim, fe_degree_rho, n_q_points_1d_rho, 1, Number> phi_rho_curr_p(data, true, 2),
                                                                         phi_rho_curr_m(data, false, 2);

      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        phi_rho_curr_p.reinit(face);
        phi_rho_curr_p.gather_evaluate(src[0], true, true);
        phi_rho_curr_m.reinit(face);
        phi_rho_curr_m.gather_evaluate(src[0], true, true);
        phi_u_curr_p.reinit(face);
        phi_u_curr_p.gather_evaluate(src[1], true, true);
        phi_u_curr_m.reinit(face);
        phi_u_curr_m.gather_evaluate(src[1], true, true);
        phi_pres_curr_p.reinit(face);
        phi_pres_curr_p.gather_evaluate(src[2], true, true);
        phi_pres_curr_m.reinit(face);
        phi_pres_curr_m.gather_evaluate(src[2], true, true);
        phi_u_tmp_2_p.reinit(face);
        phi_u_tmp_2_p.gather_evaluate(src[3], true, true);
        phi_u_tmp_2_m.reinit(face);
        phi_u_tmp_2_m.gather_evaluate(src[3], true, true);
        phi_p.reinit(face);
        phi_m.reinit(face);
        ...
        for(unsigned int q = 0; q < phi_p.n_q_points; ++q) {
          ...
          const auto& div_u_curr_p       = phi_u_curr_p.get_divergence(q);
          const auto& div_u_curr_m       = phi_u_curr_m.get_divergence(q);
          const auto& avg_tau_u_curr     = 0.5*(2.0*sym_grad_u_curr_p*u_curr_p - 2.0/3.0*div_u_curr_p*u_curr_p +
                                                2.0*sym_grad_u_curr_m*u_curr_m - 2.0/3.0*div_u_curr_m*u_curr_m);
          ...
          const auto& div_u_tmp_2_p      = phi_u_tmp_2_p.get_divergence(q);
          const auto& div_u_tmp_2_m      = phi_u_tmp_2_m.get_divergence(q);
          const auto& avg_tau_u_tmp_2    = 0.5*(2.0*sym_grad_u_tmp_2_p*u_tmp_2_p - 2.0/3.0*div_u_tmp_2_p*u_tmp_2_p +
                                                2.0*sym_grad_u_tmp_2_m*u_tmp_2_m - 2.0/3.0*div_u_tmp_2_m*u_tmp_2_m);

          const auto& rho_curr_p         = phi_rho_curr_p.get_value(q);
          const auto& rho_curr_m         = phi_rho_curr_m.get_value(q);
          const auto& grad_rho_curr_p    = phi_rho_curr_p.get_gradient(q);
          const auto& grad_rho_curr_m    = phi_rho_curr_m.get_gradient(q);
          const auto& pres_curr_p        = phi_pres_curr_p.get_value(q);
          const auto& pres_curr_m        = phi_pres_curr_m.get_value(q);
          const auto& grad_pres_curr_p   = phi_pres_curr_p.get_gradient(q);
          const auto& grad_pres_curr_m   = phi_pres_curr_m.get_gradient(q);
          const auto& avg_grad_T_curr    = 0.5*(grad_pres_curr_p/rho_curr_p - grad_rho_curr_p*pres_curr_p/(rho_curr_p*rho_curr_p) +
                                                grad_pres_curr_m/rho_curr_m - grad_rho_curr_m*pres_curr_m/(rho_curr_m*rho_curr_m));

          phi_p.submit_value(a21_tilde*dt/(Pr*Re*287.05)*scalar_product(avg_grad_T_curr, n_plus) +
                             a21_tilde*dt*Ma*Ma/Re*scalar_product(avg_tau_u_curr, n_plus) +
                             a22_tilde*dt*Ma*Ma/Re*scalar_product(avg_tau_u_tmp_2, n_plus), q);
          phi_m.submit_value(-a21_tilde*dt/(Pr*Re*287.05)*scalar_product(avg_grad_T_curr, n_plus)
                             -a21_tilde*dt*Ma*Ma/Re*scalar_product(avg_tau_u_curr, n_plus)
                             -a22_tilde*dt*Ma*Ma/Re*scalar_product(avg_tau_u_tmp_2, n_plus), q);
      ...
      FEFaceEvaluation<dim, fe_degree_u, n_q_points_1d_u, dim, Number> phi_u_curr_p(data, true, 0),
                                                                       ...
                                                                       phi_u_tmp_2_p(data, true, 0),
                                                                       phi_u_tmp_2_m(data, false, 0),
                                                                       phi_u_np1_p(data, true, 0),
                                                                       phi_u_np1_m(data, false, 0);
      FEFaceEvaluation<dim, fe_degree_rho, n_q_points_1d_rho, 1, Number> phi_rho_curr_p(data, true, 2),
                                                                         phi_rho_curr_m(data, false, 2);
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        phi_pres_tmp_2_p.gather_evaluate(src[4], true, true);
        phi_pres_tmp_2_m.reinit(face);
        phi_pres_tmp_2_m.gather_evaluate(src[4], true, true);
        phi_u_np1_p.reinit(face);
        phi_u_np1_p.gather_evaluate(src[5], true, true);
        phi_u_np1_m.reinit(face);
        phi_u_np1_m.gather_evaluate(src[5], true, true);
        phi_p.reinit(face);
        phi_m.reinit(face);
        ...
        for(unsigned int q = 0; q < phi_p.n_q_points; ++q) {
          ...
          const auto& sym_grad_u_np1_p   = phi_u_np1_p.get_symmetric_gradient(q);
          const auto& sym_grad_u_np1_m   = phi_u_np1_m.get_symmetric_gradient(q);
          const auto& u_np1_p            = phi_u_np1_p.get_value(q);
          const auto& u_np1_m            = phi_u_np1_m.get_value(q);
          const auto& div_u_np1_p        = phi_u_np1_p.get_divergence(q);
          const auto& div_u_np1_m        = phi_u_np1_m.get_divergence(q);
          const auto& avg_tau_u_np1      = 0.5*(2.0*sym_grad_u_np1_p*u_np1_p - 2.0/3.0*div_u_np1_p*u_np1_p +
                                                2.0*sym_grad_u_np1_m*u_np1_m - 2.0/3.0*div_u_np1_m*u_np1_m);
          ...
          const auto& grad_pres_tmp_2_p  = phi_pres_tmp_2_p.get_gradient(q);
          const auto& grad_pres_tmp_2_m  = phi_pres_tmp_2_m.get_gradient(q);
          const auto& avg_grad_T_tmp_2   = 0.5*(grad_pres_tmp_2_p/rho_curr_p - grad_rho_curr_p*pres_tmp_2_p/(rho_curr_p*rho_curr_p) +
                                                grad_pres_tmp_2_m/rho_curr_m - grad_rho_curr_m*pres_tmp_2_m/(rho_curr_m*rho_curr_m));

          phi_p.submit_value(a31_tilde*dt/(Pr*Re*287.05)*scalar_product(avg_grad_T_curr, n_plus) +
                             a32_tilde*dt/(Pr*Re*287.05)*scalar_product(avg_grad_T_tmp_2, n_plus) +
                             a31_tilde*dt*Ma*Ma/Re*scalar_product(avg_tau_u_curr, n_plus) +
                             a32_tilde*dt*Ma*Ma/Re*scalar_product(avg_tau_u_tmp_2, n_plus) +
                             a33_tilde*dt*Ma*Ma/Re*scalar_product(avg_tau_u_np1, n_plus), q);
          phi_m.submit_value(-a31_tilde*dt/(Pr*Re*287.05)*scalar_product(avg_grad_T_curr, n_plus) +
                             -a32_tilde*dt/(Pr*Re*287.05)*scalar_product(avg_grad_T_tmp_2, n_plus) +
                             -a31_tilde*dt*Ma*Ma/Re*scalar_product(avg_tau_u_curr, n_plus) +
                             -a32_tilde*dt*Ma*Ma/Re*scalar_product(avg_tau_u_tmp_2, n_plus) +
                             -a33_tilde*dt*Ma*Ma/Re*scalar_product(avg_tau_u_np1, n_plus), q);
  ...
  // Assemble rhs boundary term for the temperature
  //
  template<int dim, int fe_degree_rho, int fe_degree_T, int fe_degree_u,
           int n_q_points_1d_rho, int n_q_points_1d_T, int n_q_points_1d_u, typename Vec, typename Number>
  void VISCOUSOperator<dim, fe_degree_rho, fe_degree_T, fe_degree_u,
                       n_q_points_1d_rho, n_q_points_1d_T, n_q_points_1d_u, Vec, Number>::
  assemble_rhs_boundary_term_temperature(const MatrixFree<dim, Number>&               data,
                                         Vec&                                         dst,
                                         const std::vector<Vec>&                      src,
                                         const std::pair<unsigned int, unsigned int>& face_range) const {
    if(VISCOUS_stage == 1) {
      FEFaceEvaluation<dim, fe_degree_T, n_q_points_1d_T, 1, Number> phi(data, true, 1),
                                                                     phi_pres_curr(data, true, 1);
      FEFaceEvaluation<dim, fe_degree_u, n_q_points_1d_u, dim, Number> phi_u_curr(data, true, 0),
                                                                       phi_u_tmp_2(data, true, 0);
      FEFaceEvaluation<dim, fe_degree_rho, n_q_points_1d_rho, 1, Number> phi_rho_curr(data, true, 2);
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& u_curr           = phi_u_curr.get_value(q);
          const auto& div_u_curr       = phi_u_curr.get_divergence(q);
          const auto& tau_u_curr       = 2.0*sym_grad_u_curr*u_curr - 2.0/3.0*div_u_curr*u_curr;

          const auto& sym_grad_u_tmp_2 = phi_u_tmp_2.get_symmetric_gradient(q);
          const auto& u_tmp_2          = phi_u_tmp_2.get_value(q);
          const auto& div_u_tmp_2      = phi_u_tmp_2.get_divergence(q);
          const auto& tau_u_tmp_2      = 2.0*sym_grad_u_tmp_2*u_tmp_2 - 2.0/3.0*div_u_tmp_2*u_tmp_2;

          const auto& rho_curr         = phi_rho_curr.get_value(q);
          const auto& grad_rho_curr    = phi_rho_curr.get_gradient(q);
          const auto& pres_curr        = phi_pres_curr.get_value(q);
          const auto& grad_pres_curr   = phi_pres_curr.get_gradient(q);
          const auto& grad_T_curr      = grad_pres_curr/rho_curr - grad_rho_curr*pres_curr/(rho_curr*rho_curr);

          phi.submit_value(a21_tilde*dt/(Pr*Re*287.05)*scalar_product(grad_T_curr, n_plus) +
                           a21_tilde*dt*Ma*Ma/Re*scalar_product(tau_u_curr, n_plus) +
                           a22_tilde*dt*Ma*Ma/Re*scalar_product(tau_u_tmp_2, n_plus), q);
        }
        phi.integrate_scatter(true, false, dst);
      }
    }
      ...
      FEFaceEvaluation<dim, fe_degree_T, n_q_points_1d_T, 1, Number> phi(data, true, 1);
      FEFaceEvaluation<dim, fe_degree_T, n_q_points_1d_T, 1, Number> phi_pres_curr(data, true, 1),
                                                                     phi_pres_tmp_2(data, true, 1);
      FEFaceEvaluation<dim, fe_degree_u, n_q_points_1d_u, dim, Number> phi_u_curr(data, true, 0),
                                                                       phi_u_tmp_2(data, true, 0),
                                                                       phi_u_np1(data, true, 0);
      FEFaceEvaluation<dim, fe_degree_rho, n_q_points_1d_rho, 1, Number> phi_rho_curr(data, true, 2);
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& u_np1          = phi_u_np1.get_value(q);
          const auto& div_u_np1      = phi_u_np1.get_divergence(q);
          const auto& tau_u_np1      = 2.0*sym_grad_u_np1*u_np1 - 2.0/3.0*div_u_np1*u_np1;
          ...
          const auto& pres_tmp_2       = phi_pres_tmp_2.get_value(q);
          const auto& grad_pres_tmp_2  = phi_pres_tmp_2.get_gradient(q);
          const auto& grad_T_tmp_2     = grad_pres_tmp_2/rho_curr - grad_rho_curr*pres_tmp_2/(rho_curr*rho_curr);

          phi.submit_value(a31_tilde*dt/(Pr*Re*287.05)*scalar_product(grad_T_curr, n_plus) +
                           a32_tilde*dt/(Pr*Re*287.05)*scalar_product(grad_T_tmp_2, n_plus) +
                           a31_tilde*dt*Ma*Ma/Re*scalar_product(tau_u_curr, n_plus) +
                           a32_tilde*dt*Ma*Ma/Re*scalar_product(tau_u_tmp_2, n_plus) +
                           a33_tilde*dt*Ma*Ma/Re*scalar_product(tau_u_np1, n_plus), q);
        }
        phi.integrate_scatter(true, false, dst);
      }
    }
  ...
  vmult_rhs_temperature(Vec& dst, const std::vector<Vec>& src) const {
    for(unsigned int d = 0; d < src.size(); ++d)
      src[d].update_ghost_values();
    this->data->loop(&VISCOUSOperator::assemble_rhs_cell_term_temperature,
                     &VISCOUSOperator::assemble_rhs_face_term_temperature,
                     &VISCOUSOperator::assemble_rhs_boundary_term_temperature,
                     this, dst, src, true,
                     MatrixFree<dim, Number>::DataAccessOnFaces::unspecified,
                     MatrixFree<dim, Number>::DataAccessOnFaces::unspecified);
  }
  ...
  // Assemble cell term for the temperature
  //
  template<int dim, int fe_degree_rho, int fe_degree_T, int fe_degree_u,
           int n_q_points_1d_rho, int n_q_points_1d_T, int n_q_points_1d_u, typename Vec, typename Number>
  void VISCOUSOperator<dim, fe_degree_rho, fe_degree_T, fe_degree_u,
  ...
  assemble_cell_term_temperature(const MatrixFree<dim, Number>&               data,
    ...
    const double coeff = (VISCOUS_stage == 1) ? a22_tilde*dt/(Pr*Re*287.05) : a33_tilde*dt/(Pr*Re*287.05);

    for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
      phi_rho_curr.reinit(cell);
      phi_rho_curr.gather_evaluate(rho_curr, true, true);
      phi.reinit(cell);
      phi.gather_evaluate(src, true, true);
      ...
      for(unsigned int q = 0; q < phi.n_q_points; ++q) {
        ...
        phi.submit_gradient(coeff*(phi.get_gradient(q)/rho_curr - grad_rho_curr/rho_curr*phi.get_value(q)), q);
  ...
  // Assemble face term for the temperature
  //
  template<int dim, int fe_degree_rho, int fe_degree_T, int fe_degree_u,
           int n_q_points_1d_rho, int n_q_points_1d_T, int n_q_points_1d_u, typename Vec, typename Number>
  void VISCOUSOperator<dim, fe_degree_rho, fe_degree_T, fe_degree_u,
  ...
  assemble_face_term_temperature(const MatrixFree<dim, Number>&               data,
    ...
    for(unsigned int face = face_range.first; face < face_range.second; ++face) {
      phi_rho_curr_p.reinit(face);
      phi_rho_curr_p.gather_evaluate(rho_curr, true, true);
      phi_rho_curr_m.reinit(face);
      phi_rho_curr_m.gather_evaluate(rho_curr, true, true);
      phi_p.reinit(face);
      phi_p.gather_evaluate(src, true, true);
      phi_m.reinit(face);
      phi_m.gather_evaluate(src, true, true);
      ...
      for(unsigned int q = 0; q < phi_p.n_q_points; ++q) {
        ...
        const auto& avg_grad_temp = 0.5*(phi_p.get_gradient(q)/rho_curr_p - grad_rho_curr_p*phi_p.get_value(q)/(rho_curr_p*rho_curr_p) +
                                         phi_m.get_gradient(q)/rho_curr_m - grad_rho_curr_m*phi_m.get_value(q)/(rho_curr_m*rho_curr_m));

        const auto& jump_pres     = phi_p.get_value(q) - phi_m.get_value(q);

        phi_p.submit_value(-coeff*scalar_product(avg_grad_temp, n_plus) + coef_jump*jump_pres, q);
        phi_m.submit_value(coeff*scalar_product(avg_grad_temp, n_plus) - coef_jump*jump_pres, q);
        phi_p.submit_gradient(-theta_T*coeff*0.5*jump_pres*n_plus, q);
        phi_m.submit_gradient(-theta_T*coeff*0.5*jump_pres*n_plus, q);
      }
      phi_p.integrate_scatter(true, true, dst);
      phi_m.integrate_scatter(true, true, dst);
    }
  ...
  // Assemble rhs cell term for the momentum projection
  //
  template<int dim, int fe_degree_rho, int fe_degree_T, int fe_degree_u,
           int n_q_points_1d_rho, int n_q_points_1d_T, int n_q_points_1d_u, typename Vec, typename Number>
  void VISCOUSOperator<dim, fe_degree_rho, fe_degree_T, fe_degree_u,
                       n_q_points_1d_rho, n_q_points_1d_T, n_q_points_1d_u, Vec, Number>::
  assemble_rhs_cell_term_momentum_projection(const MatrixFree<dim, Number>&               data,
                                             Vec&                                         dst,
                                             const std::vector<Vec>&                      src,
                                             const std::pair<unsigned int, unsigned int>& cell_range) const {
    FEEvaluation<dim, fe_degree_u, n_q_points_1d_u, dim, Number> phi(data, 0),
  ...
  // Put together all the previous steps for momentum projection
  //
  template<int dim, int fe_degree_rho, int fe_degree_T, int fe_degree_u,
           int n_q_points_1d_rho, int n_q_points_1d_T, int n_q_points_1d_u, typename Vec, typename Number>
  void VISCOUSOperator<dim, fe_degree_rho, fe_degree_T, fe_degree_u,
                       n_q_points_1d_rho, n_q_points_1d_T, n_q_points_1d_u, Vec, Number>::
  vmult_rhs_momentum_projection(Vec& dst, const std::vector<Vec>& src) const {
    for(unsigned int d = 0; d < src.size(); ++d)
      src[d].update_ghost_values();
    this->data->cell_loop(&VISCOUSOperator::assemble_rhs_cell_term_momentum_projection,
                          this, dst, src, true);
  }


  // Assemble cell term for the momentum projection
  //
  template<int dim, int fe_degree_rho, int fe_degree_T, int fe_degree_u,
           int n_q_points_1d_rho, int n_q_points_1d_T, int n_q_points_1d_u, typename Vec, typename Number>
  void VISCOUSOperator<dim, fe_degree_rho, fe_degree_T, fe_degree_u,
                       n_q_points_1d_rho, n_q_points_1d_T, n_q_points_1d_u, Vec, Number>::
  assemble_cell_term_momentum_projection(const MatrixFree<dim, Number>&               data,
                                         Vec&                                         dst,
                                         const Vec&                                   src,
                                         const std::pair<unsigned int, unsigned int>& cell_range) const {
    FEEvaluation<dim, fe_degree_u, n_q_points_1d_u, dim, Number> phi(data, 0);
  ...
  // Assemble rhs cell term for the energy projection
  //
  template<int dim, int fe_degree_rho, int fe_degree_T, int fe_degree_u,
           int n_q_points_1d_rho, int n_q_points_1d_T, int n_q_points_1d_u, typename Vec, typename Number>
  void VISCOUSOperator<dim, fe_degree_rho, fe_degree_T, fe_degree_u,
                       n_q_points_1d_rho, n_q_points_1d_T, n_q_points_1d_u, Vec, Number>::
  assemble_rhs_cell_term_energy_projection(const MatrixFree<dim, Number>&               data,
                                             Vec&                                         dst,
                                             const std::vector<Vec>&                      src,
                                             const std::pair<unsigned int, unsigned int>& cell_range) const {
    FEEvaluation<dim, fe_degree_T, n_q_points_1d_T, 1, Number> phi(data, 1),
    ...
    for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
      ...
      for(unsigned int q = 0; q < phi.n_q_points; ++q)
        phi.submit_value(1.0/(EquationData::Cp_Cv - 1.0)*phi_pres.get_value(q) +
                         0.5*Ma*Ma*phi_rho.get_value(q)*scalar_product(phi_u.get_value(q), phi_u.get_value(q)), q);
  ...
  // Put together all the previous steps for energy projection
  //
  template<int dim, int fe_degree_rho, int fe_degree_T, int fe_degree_u,
           int n_q_points_1d_rho, int n_q_points_1d_T, int n_q_points_1d_u, typename Vec, typename Number>
  void VISCOUSOperator<dim, fe_degree_rho, fe_degree_T, fe_degree_u,
                       n_q_points_1d_rho, n_q_points_1d_T, n_q_points_1d_u, Vec, Number>::
  vmult_rhs_energy_projection(Vec& dst, const std::vector<Vec>& src) const {
    for(unsigned int d = 0; d < src.size(); ++d)
      src[d].update_ghost_values();
    this->data->cell_loop(&VISCOUSOperator::assemble_rhs_cell_term_energy_projection,
                          this, dst, src, true);
  }


  // Assemble cell term for the energy projection
  //
  template<int dim, int fe_degree_rho, int fe_degree_T, int fe_degree_u,
           int n_q_points_1d_rho, int n_q_points_1d_T, int n_q_points_1d_u, typename Vec, typename Number>
  void VISCOUSOperator<dim, fe_degree_rho, fe_degree_T, fe_degree_u,
                          n_q_points_1d_rho, n_q_points_1d_T, n_q_points_1d_u, Vec, Number>::
  assemble_cell_term_energy_projection(const MatrixFree<dim, Number>&               data,
                                         Vec&                                         dst,
                                         const Vec&                                   src,
                                         const std::pair<unsigned int, unsigned int>& cell_range) const {
    FEEvaluation<dim, fe_degree_T, n_q_points_1d_T, 1, Number> phi(data, 1);
  ...
  void VISCOUSOperator<dim, fe_degree_rho, fe_degree_T, fe_degree_u,
                       n_q_points_1d_rho, n_q_points_1d_T, n_q_points_1d_u, Vec, Number>::
  apply_add(Vec& dst, const Vec& src) const {
    AssertIndexRange(NS_stage, 5);
    Assert(NS_stage > 0, ExcInternalError());

    if(NS_stage == 1) {
      this->data->loop(&VISCOUSOperator::assemble_cell_term_velocity,
                       &VISCOUSOperator::assemble_face_term_velocity,
                       &VISCOUSOperator::assemble_boundary_term_velocity,
                       this, dst, src, false,
                       MatrixFree<dim, Number>::DataAccessOnFaces::unspecified,
                       MatrixFree<dim, Number>::DataAccessOnFaces::unspecified);
    }
    else if(NS_stage == 2){
      this->data->loop(&VISCOUSOperator::assemble_cell_term_temperature,
                       &VISCOUSOperator::assemble_face_term_temperature,
                       &VISCOUSOperator::assemble_boundary_term_temperature,
                       this, dst, src, false,
                       MatrixFree<dim, Number>::DataAccessOnFaces::unspecified,
                       MatrixFree<dim, Number>::DataAccessOnFaces::unspecified);
    }
    else if(NS_stage == 3) {
      this->data->cell_loop(&VISCOUSOperator::assemble_cell_term_momentum_projection,
                            this, dst, src, false);
    }
    else {
      this->data->cell_loop(&VISCOUSOperator::assemble_cell_term_energy_projection,
                            this, dst, src, false);
    }
  }
  ...
  protected:
    ...
    unsigned int VISCOUS_stage;    //--- Flag to check at which current stage of TR-BDF2 are
    const double Ma;
    const double Fr;
    const double Re;
    double       dt;
    ...
    LinearAlgebra::distributed::Vector<double> v_curr;
  ...
  private:
    ...
    MGLevelObject<HYPERBOLICOperator<dim, EquationData::degree_rho, EquationData::degree_T, EquationData::degree_u,
                                          2*EquationData::degree_rho + 1, 2*EquationData::degree_T + 1, 2*EquationData::degree_u + 1,
                                          LinearAlgebra::distributed::Vector<float>, float>> mg_matrices_euler;

    VISCOUSOperator<dim, EquationData::degree_rho, EquationData::degree_T, EquationData::degree_u,
                         2*EquationData::degree_rho + 1, 2*EquationData::degree_T + 1, 2*EquationData::degree_u + 1,
                         LinearAlgebra::distributed::Vector<double>, double> navier_stokes_matrix;

    MGLevelObject<VISCOUSOperator<dim, EquationData::degree_rho, EquationData::degree_T, EquationData::degree_u,
                                       2*EquationData::degree_rho + 1, 2*EquationData::degree_T + 1, 2*EquationData::degree_u + 1,
                                       LinearAlgebra::distributed::Vector<float>, float>> mg_matrices_navier_stokes;
  ...
  void NavierStokesProjection<dim>::setup_dofs() {
    ...
    mg_matrices_navier_stokes.clear_elements();
    ...
    matrix_free_storage->initialize_dof_vector(v_curr, 0);
  ...
  // This implements the diffusion step in the viscous part
  //
  template<int dim>
  void NavierStokesProjection<dim>::diffusion_step() {
    ...
    if(VISCOUS_stage == 1)
      navier_stokes_matrix.vmult_rhs_velocity(rhs_u, {rho_curr, v_curr, pres_curr});
    else
      navier_stokes_matrix.vmult_rhs_velocity(rhs_u, {rho_curr, v_curr, pres_curr,
                                                      u_tmp_2, pres_tmp_2});
    ...
    if(VISCOUS_stage == 1) {
      u_tmp_2.equ(1.0, v_curr);
      gmres.solve(navier_stokes_matrix, u_tmp_2, rhs_u, PreconditionIdentity());
    }
    else {
      u_old.equ(1.0, u_tmp_2);
      gmres.solve(navier_stokes_matrix, u_old, rhs_u, PreconditionIdentity());
    }
  }


  // @sect4{<code>NavierStokesProjection::energy</code>}

  // This implements the energy step in the viscous part
  //
  template<int dim>
  void NavierStokesProjection<dim>::energy_step() {
    TimerOutput::Scope t(time_table, "Energy step");
    ...
    if(VISCOUS_stage == 1)
      navier_stokes_matrix.vmult_rhs_temperature(rhs_pres, {rho_curr, v_curr, pres_curr, u_tmp_2});
    else
      navier_stokes_matrix.vmult_rhs_temperature(rhs_pres, {rho_curr, v_curr, pres_curr,
                                                            u_tmp_2, pres_tmp_2, u_old});

    SolverControl solver_control(vel_max_its, 1e-12*rhs_pres.l2_norm());
    SolverGMRES<LinearAlgebra::distributed::Vector<double>> gmres(solver_control);

    if(VISCOUS_stage == 1) {
      pres_tmp_2.equ(1.0, pres_curr);
      gmres.solve(navier_stokes_matrix, pres_tmp_2, rhs_pres, PreconditionIdentity());
    }
    else {
      pres_old.equ(1.0, pres_tmp_2);
      gmres.solve(navier_stokes_matrix, pres_old, rhs_pres, PreconditionIdentity());
    }
  }
  ...
  void NavierStokesProjection<dim>::project_velocity() {
    ...
    v_curr.equ(1.0, u_old);
    cg.solve(euler_matrix, v_curr, rhs_u, PreconditionIdentity());
  }


  // @sect4{<code>NavierStokesProjection::project_pressure</code>}

  // This implements the projection of the pressure in the hyperbolic part
  //
  template<int dim>
  void NavierStokesProjection<dim>::project_pressure() {
    ...
    pres_curr.equ(1.0, pres_tmp_2);
    cg.solve(euler_matrix, pres_curr, rhs_pres, PreconditionIdentity());
  }


  // @sect4{<code>NavierStokesProjection::project_momentum</code>}

  // This implements the projection of the momentum in the viscous part
  //
  template<int dim>
  void NavierStokesProjection<dim>::project_momentum() {
    TimerOutput::Scope t(time_table, "Project momentum");
    ...
    navier_stokes_matrix.vmult_rhs_momentum_projection(rhs_u, {rho_curr, u_old});

    SolverControl solver_control(vel_max_its, 1e-12*rhs_u.l2_norm());
    SolverCG<LinearAlgebra::distributed::Vector<double>> cg(solver_control);

    cg.solve(navier_stokes_matrix, u_old, rhs_u, PreconditionIdentity());
  }


  // @sect4{<code>NavierStokesProjection::project_energy</code>}

  // This implements the projection of the energy in the viscous part
  //
  template<int dim>
  void NavierStokesProjection<dim>::project_energy() {
    TimerOutput::Scope t(time_table, "Project energy");
    ...
    navier_stokes_matrix.vmult_rhs_energy_projection(rhs_pres, {rho_curr, v_curr, pres_old});

    SolverControl solver_control(vel_max_its, 1e-12*rhs_pres.l2_norm());
    SolverCG<LinearAlgebra::distributed::Vector<double>> cg(solver_control);

    cg.solve(navier_stokes_matrix, E_old, rhs_pres, PreconditionIdentity());
  }
  ...
  void NavierStokesProjection<dim>::run(const bool verbose, const unsigned int output_interval) {
    ...
    while(std::abs(T - time) > 1e-10) {
      ...
      verbose_cout << "  Update density stage 1 hyperbolic" << std::endl;
      update_density();
      verbose_cout << "  Update momentum stage 1 hyperbolic" << std::endl;
      update_momentum();
      verbose_cout << "  Update total energy stage 1 hyperbolic" << std::endl;
      update_energy();
      HYPERBOLIC_stage = 2; //--- Flag to pass at second stage

      //--- Second stage of HYPERBOLIC operator
      euler_matrix.set_HYPERBOLIC_stage(HYPERBOLIC_stage);
      verbose_cout << "  Update density stage 2 hyperbolic" << std::endl;
      update_density();
      verbose_cout << "  Update velocity stage 2 hyperbolic" << std::endl;
      update_momentum();
      verbose_cout << "  Update total energy stage 2 hyperbolic" << std::endl;
      update_energy();

      //---Project to pass to the viscous part
      project_velocity();
      project_pressure();
      VISCOUS_stage = 1;

      //--- First stage of VISCOUS operator
      navier_stokes_matrix.set_VISCOUS_stage(VISCOUS_stage);
      navier_stokes_matrix.set_rho_curr(rho_curr);
      verbose_cout << "  Update momentum stage 1 viscous" << std::endl;
      diffusion_step();
      //u_tmp_2.equ(1.0, v_curr);
      verbose_cout << "  Update energy stage 1 viscous" << std::endl;
      energy_step();
      //pres_tmp_2.equ(1.0, pres_curr);
      VISCOUS_stage = 2;

      //---Second stage of VISCOUS operator
      navier_stokes_matrix.set_VISCOUS_stage(VISCOUS_stage);
      verbose_cout << "  Update momentum stage 2 viscous" << std::endl;
      diffusion_step();
      //u_old.equ(1.0, v_curr);
      verbose_cout << "  Update energy stage 2 viscous" << std::endl;
      energy_step();
      //pres_old.equ(1.0, pres_curr);

      const double max_celerity = euler_matrix.compute_max_celerity({rho_curr, pres_old});
      ...
      //--- Project to pass to the HYPERBOLIC operator at next step
      project_momentum();
      project_energy();
      HYPERBOLIC_stage = 1;
