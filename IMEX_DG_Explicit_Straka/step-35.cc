...
namespace Step35 {
  ...
  namespace EquationData {
    ...
    double Energy<dim>::value(const Point<dim>&  p, const unsigned int component) const {
      ...
      const double x0 = 0.0;
      const double y0 = 3000.0;
      const double xr = 4000.0;
      const double yr = 2000.0;
      const double r  = std::sqrt((p[0] - x0)*(p[0] - x0)/(xr*xr) + (p[1] - y0)*(p[1] - y0)/(yr*yr));
      const double A  = -15.0;
      const double rb = 1.0;

      const double theta_prime = 0.5*A*(1.0 + std::cos(numbers::PI*r))*(r <= rb);
  ...
  void NavierStokesProjection<dim>::create_triangulation(const unsigned int n_refines) {
    ...
    upper_right[0] = 25600.0;
    upper_right[1] = 6400.0;
